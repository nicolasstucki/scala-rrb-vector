...
private[immutable] trait VectorInternal[A] {
    self: RelaxedVectorPointer[A] with VectorProps =>

    protected final def appendedBack[B >: A](elem: B): Vector[B] = {
    ...
    protected final def concatenated[U >: A](that: Vector[U]): Vector[U] = {
