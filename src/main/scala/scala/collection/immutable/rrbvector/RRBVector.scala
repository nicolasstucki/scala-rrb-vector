...
final class RRBVector[+A] private[immutable](override private[immutable] val endIndex: Int) extends scala.collection.AbstractSeq[A] with scala.collection.immutable.IndexedSeq[A] with scala.collection.generic.GenericTraversableTemplate[A, RRBVector] with scala.collection.IndexedSeqLike[A, RRBVector[A]] with RRBVectorPointer[A@uncheckedVariance] with Serializable {
    ...
    override def take(n: Int): RRBVector[A] = if (n.<=(0))
        RRBVector.empty
    else
    if (n.<(endIndex))
        takeFront0(n)
    ...
    override def dropRight(n: Int): RRBVector[A] = if (n.<=(0))
        this
    else
    if (n.<(endIndex))
...
private[immutable] trait RRBVectorPointer[A] {
    ...
    final private[immutable] def setupNewBlockInNextBranch(index: Int, xor: Int): Unit = {
        ...
        if (xor >= 1024) {
                ...
                if (xor >= 1048576) {
                    ...
                    if (xor >= 33554432) {
                        ...
                        if (xor >= 1073741824) {
                            ...
                            d5 = makeNewRoot(display4)
                            depth = 6
                        }
                        else {
                            d5 = copyAndIncRoot(display5)
                        }
                        d4 = new Array(2)
                        d5.update(index.>>(25).&(31), d4)
                        display5 = d5
                    } else if (depth.==(4)) {
                        d4 = makeNewRoot(display3)
                        depth = 5
                    } else {
                        d4 = copyAndIncRoot(display4)
                    }
                    d3 = new Array(2)
                    d4.update(index.>>(20).&(31), d3)
                    display4 = d4
                } else if (depth.==(3)) {
                    d3 = makeNewRoot(display2)
                    depth = 4
                } else {
                    d3 = copyAndIncRoot(display3)
                }
                d2 = new Array(2)
                d3.update(index.>>(15).&(31), display2)
                display3 = d3
            } else if (depth == 2) {
                d2 = makeNewRoot(display1)
                depth = 3
            }
            else {
                d2 = copyAndIncRoot(display2)
            }
            d1 = new Array(2)
            ...
            val d1 = copyOf(display1, len, len.+(1))
            val sizes = display1(len.-(1))
            if (sizes.!=(null)) {
                val newSizes = new Array[Int](len)
                Platform.arraycopy(sizes.asInstanceOf[Array[Int]], 0, newSizes, 0, len.-(1))
                newSizes.update(len.-(1), newSizes(len.-(2)).+(1))
                d1.update(len, newSizes)
            }
        }
        val d0 = new Array[AnyRef](1)
