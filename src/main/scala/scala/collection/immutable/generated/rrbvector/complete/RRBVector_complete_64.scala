package scala {
    ...
    package immutable {
      package generated {
        package rrbvector.complete {
          ...
          object RRBVector_complete_64 extends scala.collection.generic.IndexedSeqFactory[RRBVector_complete_64] {
            def newBuilder[A]: mutable.Builder[A, RRBVector_complete_64[A]] = new RRBVector_complete_64Builder[A]();
            implicit def canBuildFrom[A]: scala.collection.generic.CanBuildFrom[Coll, A, RRBVector_complete_64[A]] = ReusableCBF.asInstanceOf[GenericCanBuildFrom[A]];
            lazy private val EMPTY_VECTOR = new RRBVector_complete_64[Nothing](0);
            override def empty[A]: RRBVector_complete_64[A] = EMPTY_VECTOR
          }

          final class RRBVector_complete_64[+A](override private[immutable] val endIndex: Int) extends scala.collection.AbstractSeq[A] with scala.collection.immutable.IndexedSeq[A] with scala.collection.generic.GenericTraversableTemplate[A, RRBVector_complete_64] with scala.collection.IndexedSeqLike[A, RRBVector_complete_64[A]] with RRBVector_complete_64Pointer[A @uncheckedVariance] with Serializable { self =>
            private[immutable] var dirty: Boolean = false;
            override def companion: scala.collection.generic.GenericCompanion[RRBVector_complete_64] = RRBVector_complete_64;
            def length(): Int = endIndex;
            override def lengthCompare(len: Int): Int = endIndex.-(len);
            private[collection] def initIterator[B >: A](it: RRBVector_complete_64Iterator[B]): scala.Unit = {
              if (this.dirty)
                {
                  this.stabilize();
                  this.dirty = false
            ...
            private[collection] def initIterator[B >: A](it: RRBVector_complete_64ReverseIterator[B]): scala.Unit = {
              if (this.dirty)
                {
                  this.stabilize();
                  this.dirty = false
            ...
            override def iterator: RRBVector_complete_64Iterator[A] = {
              val it = new RRBVector_complete_64Iterator[A](0, endIndex);
              this.initIterator(it);
              it
            };
            override def reverseIterator: RRBVector_complete_64ReverseIterator[A] = {
              val it = new RRBVector_complete_64ReverseIterator[A](0, endIndex);
              this.initIterator(it);
              it
            };
            ...
            override def :+[B >: A, That](elem: B)(implicit bf: CanBuildFrom[RRBVector_complete_64[A], B, That]): That = if (bf.eq(IndexedSeq.ReusableCBF))
              (if (this.endIndex.==(0))
  {
    val resultVector = new RRBVector_complete_64[B](1);
...
else
  {
    ...
    val resultVector = new RRBVector_complete_64[B](_endIndex.+(1));
    ...
    if (elemIndexInBlock.==(0))
    ...
    else
      {
        ...
        resultVector
            ...
            override def take(n: Int): RRBVector_complete_64[A] = if (n.<=(0))
              RRBVector_complete_64.empty
            else
              if (n.<(endIndex))
                takeFront0(n)
              else
                this;
            override def dropRight(n: Int): RRBVector_complete_64[A] = if (n.<=(0))
              this
            else
              if (n.<(endIndex))
                ...
                RRBVector_complete_64.empty;
            override def slice(from: Int, until: Int): RRBVector_complete_64[A] = take(until).drop(from);
            override def splitAt(n: Int): scala.Tuple2[RRBVector_complete_64[A], RRBVector_complete_64[A]] = scala.Tuple2(take(n), drop(n));
            override def ++[B >: A, That](that: GenTraversableOnce[B])(implicit bf: CanBuildFrom[RRBVector_complete_64[A], B, That]): That = if (bf.eq(IndexedSeq.ReusableCBF))
              if (that.isEmpty)
                this.asInstanceOf[That]
              else
                if (that.isInstanceOf[RRBVector_complete_64[B]])
                  {
                    val thatVec = that.asInstanceOf[RRBVector_complete_64[B]];
                    if (this.isEmpty)
                      thatVec.asInstanceOf[That]
                    else
                      {
                        val newVec = new RRBVector_complete_64(this.endIndex.+(thatVec.endIndex));
              ...
              super.++(that.seq);
            override def tail: RRBVector_complete_64[A] = if (this.endIndex.!=(0))
              this.drop(1)
            else
            ...
            override def init: RRBVector_complete_64[A] = if (this.endIndex.!=(0))
              dropRight(1)
            else
            ...
            private[immutable] def concatenate[B >: A](currentSize: Int, that: RRBVector_complete_64[B]): scala.Unit = {
              if (this.dirty)
                {
                  this.stabilize();
                  this.dirty = false
            ...
            private def takeFront0(n: Int): RRBVector_complete_64[A] = {
              if (dirty)
                {
                  stabilize();
                  dirty = false
                ...
                ();
              val vec = new RRBVector_complete_64[A](n);
          ...
          final class RRBVector_complete_64Builder[A] extends mutable.Builder[A, RRBVector_complete_64[A]] with RRBVector_complete_64Pointer[A @uncheckedVariance] {
            ...
            def result(): RRBVector_complete_64[A] = {
              val size = endIndex;
              if (size.==(0))
                RRBVector_complete_64.empty
              else
                {
                  val resultVector = new RRBVector_complete_64[A](size);
          ...
          class RRBVector_complete_64Iterator[+A](startIndex: Int, override private[immutable] val endIndex: Int) extends AbstractIterator[A] with Iterator[A] with RRBVector_complete_64Pointer[A @uncheckedVariance] {
            private var blockIndex: Int = _;
            private var lo: Int = _;
            private var endLo: Int = _;
            private var _hasNext: Boolean = startIndex.<(endIndex);
            final private[immutable] def resetIterator(): Unit = if (_hasNext)
              {
                focusOn(startIndex);
          ...
          class RRBVector_complete_64ReverseIterator[+A](startIndex: Int, final override private[immutable] val endIndex: Int) extends AbstractIterator[A] with Iterator[A] with RRBVector_complete_64Pointer[A @uncheckedVariance] {
          ...
          private[immutable] trait RRBVector_complete_64Pointer[A] {
            ...
            final private[immutable] def initWithFocusFrom[U](that: RRBVector_complete_64[U]): Unit = {
            ...
            final private[immutable] def initFrom[U](that: RRBVector_complete_64Pointer[U]): Unit = {
              depth = that.depth;
              that.depth match {
                case 0 => ()
