...
object GenRRBVectorClosedBlocks extends scala.collection.generic.IndexedSeqFactory[GenRRBVectorClosedBlocks] {
    def newBuilder[A]: mutable.Builder[A, GenRRBVectorClosedBlocks[A]] = new GenRRBVectorClosedBlocksBuilder[A]();

    implicit def canBuildFrom[A]: scala.collection.generic.CanBuildFrom[Coll, A, GenRRBVectorClosedBlocks[A]] = ReusableCBF.asInstanceOf[GenericCanBuildFrom[A]];
    private val NIL = new GenRRBVectorClosedBlocks[Nothing](0);

    override def empty[A]: GenRRBVectorClosedBlocks[A] = NIL;

    final private[immutable] def singleton[A](value: A): GenRRBVectorClosedBlocks[A] = {
        val vec = new GenRRBVectorClosedBlocks[A](1);
...
final class GenRRBVectorClosedBlocks[+A](private[immutable] val endIndex: Int) extends scala.collection.AbstractSeq[A] with scala.collection.immutable.IndexedSeq[A] with scala.collection.generic.GenericTraversableTemplate[A, GenRRBVectorClosedBlocks] with scala.collection.IndexedSeqLike[A, GenRRBVectorClosedBlocks[A]] with GenRRBVectorClosedBlocksPointer[A@uncheckedVariance] with Serializable {
    self =>
    override def companion: scala.collection.generic.GenericCompanion[GenRRBVectorClosedBlocks] = GenRRBVectorClosedBlocks;

    def length(): Int = endIndex;

    override def lengthCompare(len: Int): Int = endIndex.-(len);

    private[collection] def initIterator[B >: A](it: GenRRBVectorClosedBlocksIterator[B]): scala.Unit = {
        it.initFrom(this);
        if (depth.>(0))
            it.resetIterator()
        else
    ...
    private[collection] def initIterator[B >: A](it: GenRRBVectorClosedBlocksReverseIterator[B]): scala.Unit = {
        it.initFrom(this);
        if (depth.>(0))
            it.resetIterator()
        else
    ...
    override def iterator: GenRRBVectorClosedBlocksIterator[A] = {
        val it = new GenRRBVectorClosedBlocksIterator[A](0, endIndex);
        this.initIterator(it);
        it
    };

    override def reverseIterator: GenRRBVectorClosedBlocksReverseIterator[A] = {
        val it = new GenRRBVectorClosedBlocksReverseIterator[A](0, endIndex);
        this.initIterator(it);
        it
    };
    ...
    override def :+[B >: A, That](elem: B)(implicit bf: CanBuildFrom[GenRRBVectorClosedBlocks[A], B, That]): That = if (bf.eq(IndexedSeq.ReusableCBF))
    ...
    override def take(n: Int): GenRRBVectorClosedBlocks[A] = if (n.<=(0))
        GenRRBVectorClosedBlocks.empty
    else
    if (n.<(endIndex))
        takeFront0(n)
    else
    ...
    override def dropRight(n: Int): GenRRBVectorClosedBlocks[A] = if (n.<=(0))
        this
    else
    if (n.<(endIndex))
    ...
    override def slice(from: Int, until: Int): GenRRBVectorClosedBlocks[A] = take(until).drop(from);

    override def splitAt(n: Int): scala.Tuple2[GenRRBVectorClosedBlocks[A], GenRRBVectorClosedBlocks[A]] = scala.Tuple2(take(n), drop(n));

    override def ++[B >: A, That](that: GenTraversableOnce[B])(implicit bf: CanBuildFrom[GenRRBVectorClosedBlocks[A], B, That]): That = if (bf.eq(IndexedSeq.ReusableCBF))
        if (that.isEmpty)
            this.asInstanceOf[That]
        else
        if (that.isInstanceOf[GenRRBVectorClosedBlocks[B]]) {
            val vec = that.asInstanceOf[GenRRBVectorClosedBlocks[B]];
            if (this.isEmpty)
    ...
    override def tail: GenRRBVectorClosedBlocks[A] = if (this.endIndex.!=(0))
        this.drop(1)
    else
    ...
    override def init: GenRRBVectorClosedBlocks[A] = if (this.endIndex.!=(0))
        dropRight(1)
    else
        throw new UnsupportedOperationException("empty.init");

    private def appendedBack[B >: A](value: B): GenRRBVectorClosedBlocks[B] = {
        if (this.endIndex.==(0))
            return GenRRBVectorClosedBlocks.singleton[B](value)
        else
            ();
        val _endIndex = this.endIndex;
        val vec = new GenRRBVectorClosedBlocks[B](_endIndex.+(1));
        vec.initFrom(this);
    ...
    private[immutable] def concatenated[B >: A](that: GenRRBVectorClosedBlocks[B]): GenRRBVectorClosedBlocks[B] = {
        this.gotoIndex(this.endIndex.-(1), this.endIndex);
        that.gotoIndex(0, that.endIndex);
        val newSize = this.endIndex.+(that.endIndex);
        val vec = new GenRRBVectorClosedBlocks[B](newSize);
        math.max(this.depth, that.depth) match {
            case 1 => {
                val concat1 = rebalancedLeafs(this.display0, that.display0);
                if (concat1.length.==(2))
                    vec.initFromRoot(concat1(0).asInstanceOf[Array[AnyRef]], 1, newSize)
                else
                    vec.initFromRoot(withComputedSizes(concat1, 1), (1).+(1), newSize)
            }
            case 2 => {
                val concat1 = rebalancedLeafs(this.display0, that.display0);
                ...
                if (concat2.length.==(2))
                    ...
                    vec.initFromRoot(withComputedSizes(concat2, 2), (2).+(1), newSize)
            }
            case 3 => {
                val concat1 = rebalancedLeafs(this.display0, that.display0);
                val concat2 = rebalanced(this.display1, concat1, that.display1, 2);
                ...
                if (concat3.length.==(2))
                    ...
                    vec.initFromRoot(withComputedSizes(concat3, 3), (3).+(1), newSize)
            }
            case 4 => {
                val concat1 = rebalancedLeafs(this.display0, that.display0);
                val concat2 = rebalanced(this.display1, concat1, that.display1, 2);
                ...
                if (concat4.length.==(2))
                    ...
                    vec.initFromRoot(withComputedSizes(concat4, 4), (4).+(1), newSize)
            }
            case 5 => {
                val concat1 = rebalancedLeafs(this.display0, that.display0);
                val concat2 = rebalanced(this.display1, concat1, that.display1, 2);
                ...
                if (concat5.length.==(2))
                    ...
                    vec.initFromRoot(withComputedSizes(concat5, 5), (5).+(1), newSize)
            }
            case 6 => {
                val concat1 = rebalancedLeafs(this.display0, that.display0);
                val concat2 = rebalanced(this.display1, concat1, that.display1, 2);
                ...
                if (concat6.length.==(2))
                    vec.initFromRoot(concat6(0).asInstanceOf[Array[AnyRef]], 6, newSize)
                else
                    vec.initFromRoot(withComputedSizes(concat6, 6), (6).+(1), newSize)
            }
            ...
            do {
                ...
                while (i.<(displayEnd)) {
                    ...
                    if (iMid.==(32)) {
                        ...
                        iTop.+=(1);
                        iMid = 0;
                        mid = new Array[AnyRef](math.min(nalen.-((32).*(iTop)), 32).+(1))
                    }
                    else
                        ()
                };
    ...
    private def rebalancedLeafs(displayLeft: Array[AnyRef], displayRight: Array[AnyRef]): Array[AnyRef] = {
    ...
    private def computeNewSizes(displayLeft: Array[AnyRef], concat: Array[AnyRef], displayRight: Array[AnyRef], currentDepth: Int) = {
            ...
            displayRight.length.-(2);
        {
            ...
            while (i.<(rightLength.-(1))) {
                ...
                if (currentDepth.==(2))
                    displayRight(i.+(1)).asInstanceOf[Array[AnyRef]].length
                else
                    displayRight(i.+(1)).asInstanceOf[Array[AnyRef]].length.-(1);
    ...
    private def takeFront0(n: Int): GenRRBVectorClosedBlocks[A] = {
        val vec = new GenRRBVectorClosedBlocks[A](n);
        vec.initFrom(this);
        if (depth.>(1)) {
            vec.gotoIndex(n.-(1), n);
...
final class GenRRBVectorClosedBlocksBuilder[A] extends mutable.Builder[A, GenRRBVectorClosedBlocks[A]] with GenRRBVectorClosedBlocksPointer[A@uncheckedVariance] {
    ...
    def result(): GenRRBVectorClosedBlocks[A] = {
        val size = blockIndex.+(lo);
        if (size.==(0))
            return GenRRBVectorClosedBlocks.empty
        else
            ();
        val resultVector = new GenRRBVectorClosedBlocks[A](size);
...
class GenRRBVectorClosedBlocksIterator[+A](startIndex: Int, endIndex: Int) extends AbstractIterator[A] with Iterator[A] with GenRRBVectorClosedBlocksPointer[A@uncheckedVariance] {
...
class GenRRBVectorClosedBlocksReverseIterator[+A](startIndex: Int, endIndex: Int) extends AbstractIterator[A] with Iterator[A] with GenRRBVectorClosedBlocksPointer[A@uncheckedVariance] {
    private var blockIndex: Int = _;
    private var lo: Int = _;
    private var endLo: Int = _;
    private var _hasNext: Boolean = startIndex.<(endIndex);
...
private[immutable] trait GenRRBVectorClosedBlocksPointer[A] {
    ...
    private[immutable] def initFrom[U](that: GenRRBVectorClosedBlocksPointer[U]): Unit = {
