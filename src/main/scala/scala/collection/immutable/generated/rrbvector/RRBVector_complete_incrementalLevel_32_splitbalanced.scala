package scala {
    ...
    package immutable {
      package generated {
        package rrbvector {
          ...
          import scala.collection.parallel.immutable.generated.rrbvector.ParRRBVector_complete_incrementalLevel_32_splitbalanced

          object RRBVector_complete_incrementalLevel_32_splitbalanced extends scala.collection.generic.IndexedSeqFactory[RRBVector_complete_incrementalLevel_32_splitbalanced] {
            def newBuilder[A]: mutable.Builder[A, RRBVector_complete_incrementalLevel_32_splitbalanced[A]] = new RRBVector_complete_incrementalLevel_32_splitbalancedBuilder[A]();
            implicit def canBuildFrom[A]: scala.collection.generic.CanBuildFrom[Coll, A, RRBVector_complete_incrementalLevel_32_splitbalanced[A]] = ReusableCBF.asInstanceOf[GenericCanBuildFrom[A]];
            lazy private val EMPTY_VECTOR = new RRBVector_complete_incrementalLevel_32_splitbalanced[Nothing](0);
            override def empty[A]: RRBVector_complete_incrementalLevel_32_splitbalanced[A] = EMPTY_VECTOR
          }

          final class RRBVector_complete_incrementalLevel_32_splitbalanced[+A](override private[immutable] val endIndex: Int) extends scala.collection.AbstractSeq[A] with scala.collection.immutable.IndexedSeq[A] with scala.collection.generic.GenericTraversableTemplate[A, RRBVector_complete_incrementalLevel_32_splitbalanced] with scala.collection.IndexedSeqLike[A, RRBVector_complete_incrementalLevel_32_splitbalanced[A]] with RRBVector_complete_incrementalLevel_32_splitbalancedPointer[A @uncheckedVariance] with Serializable { self =>
            private[immutable] var dirty: Boolean = false;
            override def companion: scala.collection.generic.GenericCompanion[RRBVector_complete_incrementalLevel_32_splitbalanced] = RRBVector_complete_incrementalLevel_32_splitbalanced;
            def length(): Int = endIndex;
            override def lengthCompare(len: Int): Int = endIndex.-(len);
            override def par = new ParRRBVector_complete_incrementalLevel_32_splitbalanced[A](this);
            override def iterator: RRBVector_complete_incrementalLevel_32_splitbalancedIterator[A] = {
              if (this.dirty)
                {
                  this.stabilize();
                  this.dirty = false
                ...
                ();
              val it = new RRBVector_complete_incrementalLevel_32_splitbalancedIterator[A](0, endIndex);
              it.initIteratorFrom(this);
              it
            };
            override def reverseIterator: RRBVector_complete_incrementalLevel_32_splitbalancedReverseIterator[A] = {
              if (this.dirty)
                {
                  this.stabilize();
                  this.dirty = false
                ...
                ();
              val rit = new RRBVector_complete_incrementalLevel_32_splitbalancedReverseIterator[A](0, endIndex);
            ...
            override def :+[B >: A, That](elem: B)(implicit bf: CanBuildFrom[RRBVector_complete_incrementalLevel_32_splitbalanced[A], B, That]): That = if (bf.eq(IndexedSeq.ReusableCBF))
              (if (this.endIndex.==(0))
  {
    val resultVector = new RRBVector_complete_incrementalLevel_32_splitbalanced[B](1);
...
else
  {
    ...
    val resultVector = new RRBVector_complete_incrementalLevel_32_splitbalanced[B](_endIndex.+(1));
    ...
    if (elemIndexInBlock.==(0))
    ...
    else
      {
        ...
        resultVector
            ...
            override def take(n: Int): RRBVector_complete_incrementalLevel_32_splitbalanced[A] = if (n.<=(0))
              RRBVector_complete_incrementalLevel_32_splitbalanced.empty
            else
              if (n.<(endIndex))
                takeFront0(n)
              else
                this;
            override def dropRight(n: Int): RRBVector_complete_incrementalLevel_32_splitbalanced[A] = if (n.<=(0))
              this
            else
              if (n.<(endIndex))
                ...
                RRBVector_complete_incrementalLevel_32_splitbalanced.empty;
            override def slice(from: Int, until: Int): RRBVector_complete_incrementalLevel_32_splitbalanced[A] = take(until).drop(from);
            override def splitAt(n: Int): scala.Tuple2[RRBVector_complete_incrementalLevel_32_splitbalanced[A], RRBVector_complete_incrementalLevel_32_splitbalanced[A]] = scala.Tuple2(take(n), drop(n));
            override def ++[B >: A, That](that: GenTraversableOnce[B])(implicit bf: CanBuildFrom[RRBVector_complete_incrementalLevel_32_splitbalanced[A], B, That]): That = if (bf.eq(IndexedSeq.ReusableCBF))
              if (that.isEmpty)
                this.asInstanceOf[That]
              else
                if (that.isInstanceOf[RRBVector_complete_incrementalLevel_32_splitbalanced[B]])
                  {
                    val thatVec = that.asInstanceOf[RRBVector_complete_incrementalLevel_32_splitbalanced[B]];
                    if (this.isEmpty)
                      thatVec.asInstanceOf[That]
                    else
                      {
                        val newVec = new RRBVector_complete_incrementalLevel_32_splitbalanced(this.endIndex.+(thatVec.endIndex));
              ...
              super.++(that.seq);
            override def tail: RRBVector_complete_incrementalLevel_32_splitbalanced[A] = if (this.endIndex.!=(0))
              this.drop(1)
            else
            ...
            override def init: RRBVector_complete_incrementalLevel_32_splitbalanced[A] = if (this.endIndex.!=(0))
              dropRight(1)
            else
            ...
            private[immutable] def concatenate[B >: A](currentSize: Int, that: RRBVector_complete_incrementalLevel_32_splitbalanced[B]): scala.Unit = {
            ...
            private def takeFront0(n: Int): RRBVector_complete_incrementalLevel_32_splitbalanced[A] = {
              if (dirty)
                {
                  stabilize();
                  dirty = false
                ...
                ();
              val vec = new RRBVector_complete_incrementalLevel_32_splitbalanced[A](n);
  ...
  val x$37 = depth.toString;
  "<=0 <==> display0==null ".+:(x$37)
}).:+(depth, display0));
              assert(depth.<=(1).&&(display1.==(null)).||(depth.>(0).&&(display1.!=(null))), ({
  val x$38 = depth.toString;
  "<=1 <==> display1==null ".+:(x$38)
}).:+(depth, display1));
              assert(depth.<=(2).&&(display2.==(null)).||(depth.>(0).&&(display2.!=(null))), ({
  val x$39 = depth.toString;
  "<=2 <==> display2==null ".+:(x$39)
}).:+(depth, display2));
              assert(depth.<=(3).&&(display3.==(null)).||(depth.>(0).&&(display3.!=(null))), ({
  val x$40 = depth.toString;
  "<=3 <==> display3==null ".+:(x$40)
}).:+(depth, display3));
              assert(depth.<=(4).&&(display4.==(null)).||(depth.>(0).&&(display4.!=(null))), ({
  val x$41 = depth.toString;
  "<=4 <==> display4==null ".+:(x$41)
}).:+(depth, display4));
              assert(depth.<=(5).&&(display5.==(null)).||(depth.>(0).&&(display5.!=(null))), ({
  val x$42 = depth.toString;
  "<=5 <==> display5==null ".+:(x$42)
}).:+(depth, display5));
              if (display5.!=(null))
                {
          ...
          final class RRBVector_complete_incrementalLevel_32_splitbalancedBuilder[A] extends mutable.Builder[A, RRBVector_complete_incrementalLevel_32_splitbalanced[A]] with RRBVector_complete_incrementalLevel_32_splitbalancedPointer[A @uncheckedVariance] {
            display0 = new Array[AnyRef](32);
            depth = 1;
            private var blockIndex = 0;
            private var lo = 0;
            private var acc: RRBVector_complete_incrementalLevel_32_splitbalanced[A] = null;
            ...
            override def ++=(xs: TraversableOnce[A]): this.type = {
              if (xs.nonEmpty)
                if (xs.isInstanceOf[RRBVector_complete_incrementalLevel_32_splitbalanced[A]])
                  {
                    val thatVec = xs.asInstanceOf[RRBVector_complete_incrementalLevel_32_splitbalanced[A]];
                    if (endIndex.!=(0))
                      {
                        acc = this.result().++(xs);
                        this.clearCurrent()
            ...
            def result(): RRBVector_complete_incrementalLevel_32_splitbalanced[A] = {
            ...
            private def currentResult(): RRBVector_complete_incrementalLevel_32_splitbalanced[A] = {
              val size = blockIndex.+(lo);
              if (size.==(0))
                RRBVector_complete_incrementalLevel_32_splitbalanced.empty
              else
                {
                  val resultVector = new RRBVector_complete_incrementalLevel_32_splitbalanced[A](size);
          ...
          class RRBVector_complete_incrementalLevel_32_splitbalancedIterator[+A](startIndex: Int, override private[immutable] val endIndex: Int) extends AbstractIterator[A] with Iterator[A] with RRBVector_complete_incrementalLevel_32_splitbalancedPointer[A @uncheckedVariance] {
            private var blockIndex: Int = _;
            private var lo: Int = _;
            private var endLo: Int = _;
            private var _hasNext: Boolean = _;
            final private[collection] def initIteratorFrom[B >: A](that: RRBVector_complete_incrementalLevel_32_splitbalancedPointer[B]): Unit = {
              if (_hasNext)
                {
                  focusOn(startIndex);
          ...
          class RRBVector_complete_incrementalLevel_32_splitbalancedReverseIterator[+A](startIndex: Int, final override private[immutable] val endIndex: Int) extends AbstractIterator[A] with Iterator[A] with RRBVector_complete_incrementalLevel_32_splitbalancedPointer[A @uncheckedVariance] {
          ...
          private[immutable] trait RRBVector_complete_incrementalLevel_32_splitbalancedPointer[A] {
            ...
            final private[immutable] def initWithFocusFrom[U](that: RRBVector_complete_incrementalLevel_32_splitbalancedPointer[U]): Unit = {
            ...
            final private[immutable] def initFrom[U](that: RRBVector_complete_incrementalLevel_32_splitbalancedPointer[U]): Unit = {
              assert(that.!=(null));
              depth = that.depth;
              that.depth match {
                case 0 => ()
      ...
      package immutable {
        package generated {
          package rrbvector {
            ...
            class ParRRBVector_complete_incrementalLevel_32_splitbalanced[+A](vector: RRBVector_complete_incrementalLevel_32_splitbalanced[A]) extends ParSeq[A] with GenericParTemplate[A, ParRRBVector_complete_incrementalLevel_32_splitbalanced] with ParSeqLike[A, ParRRBVector_complete_incrementalLevel_32_splitbalanced[A], RRBVector_complete_incrementalLevel_32_splitbalanced[A]] with Serializable {
              override def companion = ParRRBVector_complete_incrementalLevel_32_splitbalanced;
              def this() = {
                this(RRBVector_complete_incrementalLevel_32_splitbalanced.empty[A]);
                ()
              };
              def apply(idx: Int) = vector.apply(idx);
              def length = vector.length;
              def splitter: SeqSplitter[A] = {
                val pit = new ParRRBVector_complete_incrementalLevel_32_splitbalancedIterator(0, vector.length);
                pit.initIteratorFrom(vector);
                pit
              };
              override def seq: RRBVector_complete_incrementalLevel_32_splitbalanced[A] = vector;
              override def toVector: Vector[A] = vector.toVector;
              class ParRRBVector_complete_incrementalLevel_32_splitbalancedIterator(_start: Int, _end: Int) extends RRBVector_complete_incrementalLevel_32_splitbalancedIterator[A](_start, _end) with SeqSplitter[A] {
                final override def remaining: Int = super.remaining;
                def dup: SeqSplitter[A] = {
                  val pit = new ParRRBVector_complete_incrementalLevel_32_splitbalancedIterator(_end.-(remaining), _end);
                  pit.initIteratorFrom(this);
                  pit
                };
                def split: Seq[ParRRBVector_complete_incrementalLevel_32_splitbalancedIterator] = {
                  val rem = remaining;
                  if (rem.>=(2))
                    {
                      val splitSize = (1).<<((5).*((31).-(java.lang.Integer.numberOfLeadingZeros(rem))./(5).-(1)));
                      val splitted = new ArrayBuffer[ParRRBVector_complete_incrementalLevel_32_splitbalancedIterator]();
                      var currentPos = _end.-(remaining);
                      while (currentPos.<(rem)) 
                        {
                          val pit = new ParRRBVector_complete_incrementalLevel_32_splitbalancedIterator(currentPos, math.min(currentPos.+(splitSize), _end));
                          pit.initIteratorFrom(this);
                          splitted.+=(pit);
                ...
                def psplit(sizes: Int*): Seq[ParRRBVector_complete_incrementalLevel_32_splitbalancedIterator] = {
                  val splitted = new ArrayBuffer[ParRRBVector_complete_incrementalLevel_32_splitbalancedIterator]();
                  var currentPos = _end.-(remaining);
                  sizes.foreach(((sz) => {
                    val pit = new ParRRBVector_complete_incrementalLevel_32_splitbalancedIterator(currentPos, currentPos.+(sz));
                    pit.initIteratorFrom(this);
                    splitted.+=(pit);
                    currentPos.+=(sz)
                  }));
            ...
            object ParRRBVector_complete_incrementalLevel_32_splitbalanced extends ParFactory[ParRRBVector_complete_incrementalLevel_32_splitbalanced] {
              implicit def canBuildFrom[A]: CanCombineFrom[Coll, A, ParRRBVector_complete_incrementalLevel_32_splitbalanced[A]] = new GenericCanCombineFrom[A]();
              def newBuilder[A]: Combiner[A, ParRRBVector_complete_incrementalLevel_32_splitbalanced[A]] = newCombiner[A];
              def newCombiner[A]: Combiner[A, ParRRBVector_complete_incrementalLevel_32_splitbalanced[A]] = new ParRRBVector_complete_incrementalLevel_32_splitbalancedCombinator[A]()
            }

            private[immutable] class ParRRBVector_complete_incrementalLevel_32_splitbalancedCombinator[A] extends Combiner[A, ParRRBVector_complete_incrementalLevel_32_splitbalanced[A]] {
              val builder: RRBVector_complete_incrementalLevel_32_splitbalancedBuilder[A] = new RRBVector_complete_incrementalLevel_32_splitbalancedBuilder[A]();
              override def size = builder.endIndex;
              override def result() = new ParRRBVector_complete_incrementalLevel_32_splitbalanced[A](builder.result());
              ...
              def combine[B <: A, NewTo >: ParRRBVector_complete_incrementalLevel_32_splitbalanced[A]](other: Combiner[B, NewTo]) = if (other.eq(this))
                this
              else
                {
                  val that = other.asInstanceOf[ParRRBVector_complete_incrementalLevel_32_splitbalancedCombinator[B]];
