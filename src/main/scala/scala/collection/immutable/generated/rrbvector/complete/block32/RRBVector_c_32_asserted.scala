          ...
          package rrbvector.complete.block32 {
                  ...
                  if (rem.>=(2))
                    {
                      ...
                      while (currentPos.<(rem)) 
                        {
                          val pit = new ParRRBVectorIterator_c_32_asserted(currentPos, math.min(currentPos.+(splitSize), _end));
                          pit.initIteratorFrom(this);
                          splitted.+=(pit);
            ...
            private[immutable] class ParRRBVectorCombinator_c_32_asserted[A] extends Combiner[A, ParRRBVector_c_32_asserted[A]] {
              ...
              def combine[B <: A, NewTo >: ParRRBVector_c_32_asserted[A]](other: Combiner[B, NewTo]) = if (other.eq(this))
                ...
                {
                  val that = other.asInstanceOf[ParRRBVectorCombinator_c_32_asserted[B]];
