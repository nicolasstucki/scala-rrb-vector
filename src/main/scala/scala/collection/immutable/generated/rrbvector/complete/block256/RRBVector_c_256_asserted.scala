package scala {
    ...
    package immutable {
      package generated {
        package rrbvector.complete.block256 {
          ...
          import scala.collection.parallel.immutable.generated.rrbvector.complete.block256.ParRRBVector_c_256_asserted

          object RRBVector_c_256_asserted extends scala.collection.generic.IndexedSeqFactory[RRBVector_c_256_asserted] {
            def newBuilder[A]: mutable.Builder[A, RRBVector_c_256_asserted[A]] = new RRBVectorBuilder_c_256_asserted[A]();
            implicit def canBuildFrom[A]: scala.collection.generic.CanBuildFrom[Coll, A, RRBVector_c_256_asserted[A]] = ReusableCBF.asInstanceOf[GenericCanBuildFrom[A]];
            lazy private val EMPTY_VECTOR = new RRBVector_c_256_asserted[Nothing](0);
            override def empty[A]: RRBVector_c_256_asserted[A] = EMPTY_VECTOR
          }

          final class RRBVector_c_256_asserted[+A](override private[immutable] val endIndex: Int) extends scala.collection.AbstractSeq[A] with scala.collection.immutable.IndexedSeq[A] with scala.collection.generic.GenericTraversableTemplate[A, RRBVector_c_256_asserted] with scala.collection.IndexedSeqLike[A, RRBVector_c_256_asserted[A]] with RRBVectorPointer_c_256_asserted[A @uncheckedVariance] with Serializable { self =>
            private[immutable] var dirty: Boolean = false;
            override def companion: scala.collection.generic.GenericCompanion[RRBVector_c_256_asserted] = RRBVector_c_256_asserted;
            def length(): Int = endIndex;
            override def lengthCompare(len: Int): Int = endIndex.-(len);
            override def par = new ParRRBVector_c_256_asserted[A](this);
            override def iterator: RRBVectorIterator_c_256_asserted[A] = {
              if (this.dirty)
                {
                  this.stabilize();
                  this.dirty = false
                ...
                ();
              val it = new RRBVectorIterator_c_256_asserted[A](0, endIndex);
              it.initIteratorFrom(this);
              it
            };
            override def reverseIterator: RRBVectorReverseIterator_c_256_asserted[A] = {
              if (this.dirty)
                {
                  this.stabilize();
                  this.dirty = false
                ...
                ();
              val rit = new RRBVectorReverseIterator_c_256_asserted[A](0, endIndex);
            ...
            override def :+[B >: A, That](elem: B)(implicit bf: CanBuildFrom[RRBVector_c_256_asserted[A], B, That]): That = if (bf.eq(IndexedSeq.ReusableCBF))
              (if (this.endIndex.==(0))
  {
    val resultVector = new RRBVector_c_256_asserted[B](1);
...
else
  {
    ...
    val resultVector = new RRBVector_c_256_asserted[B](_endIndex.+(1));
    resultVector.initWithFocusFrom(this);
    resultVector.dirty = this.dirty;
    if (_endIndex.-(1).-(this.focusStart).^(this.focus).>=(256))
      {
      ...
      ();
    val elemIndexInBlock = _endIndex.-(resultVector.focusStart).&(255);
    if (elemIndexInBlock.==(0))
    ...
    else
      {
        ...
        resultVector
            ...
            override def take(n: Int): RRBVector_c_256_asserted[A] = if (n.<=(0))
              RRBVector_c_256_asserted.empty
            else
              if (n.<(endIndex))
                takeFront0(n)
              else
                this;
            override def dropRight(n: Int): RRBVector_c_256_asserted[A] = if (n.<=(0))
              this
            else
              if (n.<(endIndex))
                ...
                RRBVector_c_256_asserted.empty;
            override def slice(from: Int, until: Int): RRBVector_c_256_asserted[A] = take(until).drop(from);
            override def splitAt(n: Int): scala.Tuple2[RRBVector_c_256_asserted[A], RRBVector_c_256_asserted[A]] = scala.Tuple2(take(n), drop(n));
            override def ++[B >: A, That](that: GenTraversableOnce[B])(implicit bf: CanBuildFrom[RRBVector_c_256_asserted[A], B, That]): That = if (bf.eq(IndexedSeq.ReusableCBF))
              if (that.isEmpty)
                this.asInstanceOf[That]
              else
                if (that.isInstanceOf[RRBVector_c_256_asserted[B]])
                  {
                    val thatVec = that.asInstanceOf[RRBVector_c_256_asserted[B]];
                    if (this.isEmpty)
                      thatVec.asInstanceOf[That]
                    else
                      {
                        val newVec = new RRBVector_c_256_asserted(this.endIndex.+(thatVec.endIndex));
              ...
              super.++(that.seq);
            override def tail: RRBVector_c_256_asserted[A] = if (this.endIndex.!=(0))
              this.drop(1)
            else
            ...
            override def init: RRBVector_c_256_asserted[A] = if (this.endIndex.!=(0))
              dropRight(1)
            else
            ...
            private[immutable] def appendBackSetupNewBlock() = {
              ...
              if (oldDepth.==(depth))
                {
                  var i = (31).-(java.lang.Integer.numberOfLeadingZeros(xor))./(8).+(1);
                  ...
                  while (i.<(oldDepth)) 
                    {
                      ...
                      i match {
                        case 2 => {
                          newDisplay.update(newRelaxedIndex.>>(16).&(255), display1);
                          display2 = newDisplay
                        }
                        case 3 => {
                          newDisplay.update(newRelaxedIndex.>>(24).&(255), display2);
                          display3 = newDisplay
                        }
                        case 4 => {
                          newDisplay.update(newRelaxedIndex.>>(32).&(255), display3);
                          display4 = newDisplay
                        }
                        case 5 => {
                          newDisplay.update(newRelaxedIndex.>>(40).&(255), display4);
                          display5 = newDisplay
                        }
                      };
                      i.+=(1)
              ...
              if (oldDepth.==(focusDepth))
                ...
                initFocus(endIndex.-(1), endIndex.-(1), endIndex, 1, newRelaxedIndex.&(-256))
            };
            private[immutable] def concatenate[B >: A](currentSize: Int, that: RRBVector_c_256_asserted[B]): scala.Unit = {
              ...
              math.max(this.depth, that.depth) match {
                case 1 => {
                  ...
                  initFromRoot(concat, if (endIndex.<=(256))
                    1
                  else
                    2);
            ...
            private def rebalanced(displayLeft: Array[AnyRef], concat: Array[AnyRef], displayRight: Array[AnyRef], currentDepth: Int): Array[AnyRef] = {
              ...
              val top = new Array[AnyRef](branching.>>(16).+(if (branching.&(65535).==(0))
                1
              else
                2));
              var mid = new Array[AnyRef](if (branching.>>(16).==(0))
                branching.+(255).>>(8).+(1)
              else
                257);
                  ...
                  while (i.<(displayEnd)) 
                    {
                      ...
                      val displayValueEnd = displayValue.length.-(if (currentDepth.==(2))
                        ...
                        1);
                      if (iBot.|(j).==(0).&&(displayValueEnd.==(256)))
                        {
                        ...
                        {
                          val numElementsToCopy = math.min(displayValueEnd.-(j), (256).-(iBot));
                          if (iBot.==(0))
                            {
                              if (currentDepth.!=(2).&&(bot.!=(null)))
                                withComputedSizes(bot, currentDepth.-(1))
                                ...
                                ();
                              bot = new Array[AnyRef](math.min(branching.-(iTop.<<(16)).-(iMid.<<(8)), 256).+(if (currentDepth.==(2))
                                0
                              else
                                1));
                              mid.update(iMid, bot)
                      ...
                      if (iMid.==(256))
                        {
                          ...
                          iTop.+=(1);
                          iMid = 0;
                          val remainingBranches = branching.-(iTop.<<(8).|(iMid).<<(8).|(iBot));
                          if (remainingBranches.>(0))
                            mid = new Array[AnyRef](if (remainingBranches.>>(16).==(0))
                              remainingBranches.+(511).>>(8)
                            else
                              257)
                          else
                            mid = null
            ...
            private def rebalancedLeafs(displayLeft: Array[AnyRef], displayRight: Array[AnyRef], isTop: Boolean): Array[AnyRef] = {
              ...
              if (leftLength.==(256))
                ...
                if (leftLength.+(rightLength).<=(256))
                  ...
                  {
                    val top = new Array[AnyRef](3);
                    val arr0 = new Array[AnyRef](256);
                    val arr1 = new Array[AnyRef](leftLength.+(rightLength).-(256));
                    top.update(0, arr0);
                    top.update(1, arr1);
                    Platform.arraycopy(displayLeft, 0, arr0, 0, leftLength);
                    Platform.arraycopy(displayRight, 0, arr0, leftLength, (256).-(leftLength));
                    Platform.arraycopy(displayRight, (256).-(leftLength), arr1, 0, rightLength.-(256).+(leftLength));
                    top
                  }
            };
              ...
              if (currentDepth.>(1))
                {
                  ...
                  if (end.>(1).&&(sizes(end.-(2)).!=(end.-(1).<<((8).*(currentDepth.-(1))))).||(currentDepth.>(2).&&(last(last.length.-(1)).!=(null))))
                  ...
                  if (treeSizes.!=(null))
                  ...
                  else
                    {
                      ...
                      while (_currentDepth.>(1)) 
                        {
                          acc.+=(_tree.length.-(2).*((1).<<((8).*(_currentDepth.-(1)))));
                          _currentDepth.-=(1);
            ...
            private def takeFront0(n: Int): RRBVector_c_256_asserted[A] = {
              if (dirty)
                {
                  stabilize();
                  dirty = false
                ...
                ();
              val vec = new RRBVector_c_256_asserted[A](n);
              vec.initWithFocusFrom(this);
              if (depth.>(1))
                {
                  vec.focusOn(n.-(1));
                  val d0len = vec.focus.&(255).+(1);
                  if (d0len.!=(256))
                    {
                      ...
                      while (i.>(vec.focusDepth)) 
                        {
                          ...
                          val newLen = vec.focusRelax.>>((8).*(i.-(1))).&(255).+(1);
  ...
  val x$19 = depth.toString;
  "<=0 <==> display0==null ".+:(x$19)
}).:+(depth, display0));
              assert(depth.<=(1).&&(display1.==(null)).||(depth.>(0).&&(display1.!=(null))), ({
  val x$20 = depth.toString;
  "<=1 <==> display1==null ".+:(x$20)
}).:+(depth, display1));
              assert(depth.<=(2).&&(display2.==(null)).||(depth.>(0).&&(display2.!=(null))), ({
  val x$21 = depth.toString;
  "<=2 <==> display2==null ".+:(x$21)
}).:+(depth, display2));
              assert(depth.<=(3).&&(display3.==(null)).||(depth.>(0).&&(display3.!=(null))), ({
  val x$22 = depth.toString;
  "<=3 <==> display3==null ".+:(x$22)
}).:+(depth, display3));
              assert(depth.<=(4).&&(display4.==(null)).||(depth.>(0).&&(display4.!=(null))), ({
  val x$23 = depth.toString;
  "<=4 <==> display4==null ".+:(x$23)
}).:+(depth, display4));
              assert(depth.<=(5).&&(display5.==(null)).||(depth.>(0).&&(display5.!=(null))), ({
  val x$24 = depth.toString;
  "<=5 <==> display5==null ".+:(x$24)
}).:+(depth, display5));
              if (display5.!=(null))
                {
                  assert(display4.!=(null));
                  if (focusDepth.<=(5))
                    assert(display5(focusRelax.>>(40).&(255)).==(display4))
                  else
                    assert(display5(focus.>>(40).&(255)).==(display4))
                }
              else
                ();
              if (display4.!=(null))
                {
                  ...
                  if (focusDepth.<=(4))
                    assert(display4(focusRelax.>>(32).&(255)).==(display3))
                  else
                    assert(display4(focus.>>(32).&(255)).==(display3))
                }
              else
                ();
              if (display3.!=(null))
                {
                  ...
                  if (focusDepth.<=(3))
                    assert(display3(focusRelax.>>(24).&(255)).==(display2))
                  else
                    assert(display3(focus.>>(24).&(255)).==(display2))
                }
              else
                ();
              if (display2.!=(null))
                {
                  ...
                  if (focusDepth.<=(2))
                    assert(display2(focusRelax.>>(16).&(255)).==(display1))
                  else
                    assert(display2(focus.>>(16).&(255)).==(display1))
                }
              else
                ();
              if (display1.!=(null))
                {
                  ...
                  if (dirty.`unary_!`)
                    if (focusDepth.<=(1))
                      assert(display1(focusRelax.>>(8).&(255)).==(display0))
                    else
                      assert(display1(focus.>>(8).&(255)).==(display0))
                  else
                    ()
                }
              else
              ...
              def checkSizes(node: Array[AnyRef], currentDepth: Int, _endIndex: Int): Unit = if (currentDepth.>(1))
                {
                  ...
                  if (sizes.!=(null))
                    ...
                    {
                      0.until(node.length.-(2)).foreach(((i) => checkSizes(node(i).asInstanceOf[Array[AnyRef]], currentDepth.-(1), (1).<<((8).*(currentDepth.-(1))))));
                      val expectedLast = _endIndex.-((1).<<((8).*(currentDepth.-(1))).*(node.length.-(2)));
                      assert((1).<=(expectedLast).&&(expectedLast.<=((1).<<((8).*(currentDepth)))));
          ...
          final class RRBVectorBuilder_c_256_asserted[A] extends mutable.Builder[A, RRBVector_c_256_asserted[A]] with RRBVectorPointer_c_256_asserted[A @uncheckedVariance] {
            display0 = new Array[AnyRef](256);
            depth = 1;
            private var blockIndex = 0;
            private var lo = 0;
            private var acc: RRBVector_c_256_asserted[A] = null;
            def +=(elem: A): this.type = {
              if (lo.>=(256))
                {
                  val newBlockIndex = blockIndex.+(256);
            ...
            override def ++=(xs: TraversableOnce[A]): this.type = {
              if (xs.nonEmpty)
                if (xs.isInstanceOf[RRBVector_c_256_asserted[A]])
                  {
                    val thatVec = xs.asInstanceOf[RRBVector_c_256_asserted[A]];
                    if (endIndex.!=(0))
                      {
                        acc = this.result().++(xs);
                        this.clearCurrent()
            ...
            def result(): RRBVector_c_256_asserted[A] = {
            ...
            private def currentResult(): RRBVector_c_256_asserted[A] = {
              val size = blockIndex.+(lo);
              if (size.==(0))
                RRBVector_c_256_asserted.empty
              else
                {
                  val resultVector = new RRBVector_c_256_asserted[A](size);
          ...
          class RRBVectorIterator_c_256_asserted[+A](startIndex: Int, override private[immutable] val endIndex: Int) extends AbstractIterator[A] with Iterator[A] with RRBVectorPointer_c_256_asserted[A @uncheckedVariance] {
            private var blockIndex: Int = _;
            private var lo: Int = _;
            private var endLo: Int = _;
            private var _hasNext: Boolean = _;
            final private[collection] def initIteratorFrom[B >: A](that: RRBVectorPointer_c_256_asserted[B]): Unit = {
              if (_hasNext)
                {
                  focusOn(startIndex);
                  blockIndex = focusStart.+(focus.&(-256));
                  lo = focus.&(255);
                  endLo = math.min(focusEnd.-(blockIndex), 256)
                }
              else
                {
                  blockIndex = 0;
            ...
            def next(): A = {
              ...
              if (_lo.+(1).!=(_endLo))
              ...
              else
                {
                  ...
                  endLo = math.min(focusEnd.-(newBlockIndex), 256);
                  res
                }
            };
          ...
          class RRBVectorReverseIterator_c_256_asserted[+A](startIndex: Int, final override private[immutable] val endIndex: Int) extends AbstractIterator[A] with Iterator[A] with RRBVectorPointer_c_256_asserted[A @uncheckedVariance] {
            ...
            def next(): A = if (_hasNext)
              {
                ...
                if (lo.>=(endLo))
                  ...
                  {
                    val newBlockIndex = lastIndexOfBlock.-(256);
          ...
          private[immutable] trait RRBVectorPointer_c_256_asserted[A] {
            ...
            final private[immutable] def initWithFocusFrom[U](that: RRBVectorPointer_c_256_asserted[U]): Unit = {
            ...
            final private[immutable] def initFrom[U](that: RRBVectorPointer_c_256_asserted[U]): Unit = {
              assert(that.!=(null));
              depth = that.depth;
              that.depth match {
                case 0 => ()
              ...
              while (currentDepth.>(1)) 
                {
                  ...
                  if (sizes.==(null))
                    {
                      val depthShift = (8).*(currentDepth.-(1));
                      ...
                      if (sizes.==(null))
                      ...
                      else
                        {
                          ...
                          if (is.!=(0))
                            ...
                            ();
                          currentDepth.-=(1);
                          _focusRelax.|=(is.<<((8).*(currentDepth)))
                        }
                    }
                }
              ;
              val indexInFocus = index.-(_startIndex);
              gotoPos(indexInFocus, (1).<<((8).*(currentDepth.-(1))));
              initFocus(indexInFocus, _startIndex, _endIndex, currentDepth, _focusRelax)
            };
            final private[immutable] def setupNewBlockInNextBranch(index: Int, xor: Int): Unit = if (xor.<(65536))
              {
                if (depth.==(1))
                  {
                ...
                display1.update(index.>>(8).&(255), display0)
              }
            else
              if (xor.<(16777216))
                {
                  ...
                  display1.update(index.>>(8).&(255), display0);
                  display2.update(index.>>(16).&(255), display1)
                }
              else
                if (xor.<(1))
                  {
                    ...
                    display1.update(index.>>(8).&(255), display0);
                    display2.update(index.>>(16).&(255), display1);
                    display3.update(index.>>(24).&(255), display2)
                  }
                else
                  if (xor.<(256))
                    {
                      ...
                      display2.update(index.>>(16).&(255), display1);
                      display3.update(index.>>(24).&(255), display2);
                      display4.update(index.>>(32).&(255), display3)
                    }
                  else
                    if (xor.<(65536))
                      {
                        ...
                        display3.update(index.>>(24).&(255), display2);
                        display4.update(index.>>(32).&(255), display3);
                        display5.update(index.>>(40).&(255), display4)
                      }
                    else
                      throw new IllegalArgumentException();
            final private[immutable] def getElement(index: Int, xor: Int): A = if (xor.<(256))
              display0(index.&(255)).asInstanceOf[A]
            else
              if (xor.<(65536))
                display1(index.>>(8).&(255)).asInstanceOf[Array[AnyRef]](index.&(255)).asInstanceOf[A]
              else
                if (xor.<(16777216))
                  display2(index.>>(16).&(255)).asInstanceOf[Array[AnyRef]](index.>>(8).&(255)).asInstanceOf[Array[AnyRef]](index.&(255)).asInstanceOf[A]
                else
                  if (xor.<(1))
                    display3(index.>>(24).&(255)).asInstanceOf[Array[AnyRef]](index.>>(16).&(255)).asInstanceOf[Array[AnyRef]](index.>>(8).&(255)).asInstanceOf[Array[AnyRef]](index.&(255)).asInstanceOf[A]
                  else
                    if (xor.<(256))
                      display4(index.>>(32).&(255)).asInstanceOf[Array[AnyRef]](index.>>(24).&(255)).asInstanceOf[Array[AnyRef]](index.>>(16).&(255)).asInstanceOf[Array[AnyRef]](index.>>(8).&(255)).asInstanceOf[Array[AnyRef]](index.&(255)).asInstanceOf[A]
                    else
                      if (xor.<(65536))
                        display5(index.>>(40).&(255)).asInstanceOf[Array[AnyRef]](index.>>(32).&(255)).asInstanceOf[Array[AnyRef]](index.>>(24).&(255)).asInstanceOf[Array[AnyRef]](index.>>(16).&(255)).asInstanceOf[Array[AnyRef]](index.>>(8).&(255)).asInstanceOf[Array[AnyRef]](index.&(255)).asInstanceOf[A]
                      else
                        throw new IllegalArgumentException();
            final private[immutable] def gotoPos(index: Int, xor: Int): Unit = if (xor.>=(256))
              {
                val d1 = if (xor.>=(65536))
                  {
                    val d2 = if (xor.>=(16777216))
                      {
                        val d3 = if (xor.>=(1))
                          {
                            val d4 = if (xor.>=(256))
                              {
                                if (xor.>=(1073741824))
                                  ...
                                  ();
                                val _d4 = display5(index.>>(40).&(255)).asInstanceOf[Array[AnyRef]];
                                display4 = _d4;
                                _d4
                              }
                            else
                              display4;
                            val _d3 = d4(index.>>(32).&(255)).asInstanceOf[Array[AnyRef]];
                            display3 = _d3;
                            _d3
                          }
                        else
                          display3;
                        val _d2 = d3(index.>>(24).&(255)).asInstanceOf[Array[AnyRef]];
                        display2 = _d2;
                        _d2
                      }
                    else
                      display2;
                    val _d1 = d2(index.>>(16).&(255)).asInstanceOf[Array[AnyRef]];
                    display1 = _d1;
                    _d1
                  }
                else
                  display1;
                display0 = d1(index.>>(8).&(255)).asInstanceOf[Array[AnyRef]]
              }
            else
              ();
                          ...
                          if (xor.>=(256))
                            {
                              if (xor.>=(65536))
                                ...
                                display4 = display5(index.>>(40).&(255)).asInstanceOf[Array[AnyRef]];
                              idx = 0
                            }
                          else
                            idx = index.>>(32).&(255);
            ...
            final private[immutable] def gotoNextBlockStartWritable(index: Int, xor: Int): Unit = if (xor.<(65536))
              {
                if (depth.==(1))
                  {
                    display1 = new Array(257);
                    display1.update(0, display0);
                    depth.+=(1)
                  }
                else
                  ();
                display0 = new Array(256);
                display1.update(index.>>(8).&(255), display0)
              }
            else
              if (xor.<(16777216))
              ...
              if (_depth.>(1))
                {
                  ...
                  val deltaSize = display0.length.-(display1(stabilizationIndex.>>(8).&(255)).asInstanceOf[Array[AnyRef]].length);
                  val _focusDepth = focusDepth;
                  ...
                  while (currentDepth.<=(_depth)) 
                    {
                      ...
                      val idx = stabilizationIndex.>>((8).*(currentDepth)).&(255);
            ...
            final private[immutable] def stabilizeDisplayPath(_depth: Int, _focus: Int): Unit = {
              ...
              _depth match {
                case 1 => ()
                case 2 => display1.update(_focus.>>(8).&(255), display0)
                case 3 => {
                  display2.update(_focus.>>(16).&(255), display1);
                  display1.update(_focus.>>(8).&(255), display0)
                }
                case 4 => {
                  display3.update(_focus.>>(24).&(255), display2);
                  display2.update(_focus.>>(16).&(255), display1);
                  display1.update(_focus.>>(8).&(255), display0)
                }
                case 5 => {
                  display4.update(_focus.>>(32).&(255), display3);
                  display3.update(_focus.>>(24).&(255), display2);
                  display2.update(_focus.>>(16).&(255), display1);
                  display1.update(_focus.>>(8).&(255), display0)
                }
                case 6 => {
                  display5.update(_focus.>>(40).&(255), display4);
                  display4.update(_focus.>>(32).&(255), display3);
                  display3.update(_focus.>>(24).&(255), display2);
                  display2.update(_focus.>>(16).&(255), display1);
                  display1.update(_focus.>>(8).&(255), display0)
      ...
      package immutable {
        package generated {
          package rrbvector.complete.block256 {
            ...
            class ParRRBVector_c_256_asserted[+A](vector: RRBVector_c_256_asserted[A]) extends ParSeq[A] with GenericParTemplate[A, ParRRBVector_c_256_asserted] with ParSeqLike[A, ParRRBVector_c_256_asserted[A], RRBVector_c_256_asserted[A]] with Serializable {
              override def companion = ParRRBVector_c_256_asserted;
              def this() = {
                this(RRBVector_c_256_asserted.empty[A]);
                ()
              };
              def apply(idx: Int) = vector.apply(idx);
              def length = vector.length;
              def splitter: SeqSplitter[A] = {
                val pit = new ParRRBVectorIterator_c_256_asserted(0, vector.length);
                pit.initIteratorFrom(vector);
                pit
              };
              override def seq: RRBVector_c_256_asserted[A] = vector;
              override def toVector: Vector[A] = vector.toVector;
              class ParRRBVectorIterator_c_256_asserted(_start: Int, _end: Int) extends RRBVectorIterator_c_256_asserted[A](_start, _end) with SeqSplitter[A] {
                final override def remaining: Int = super.remaining;
                def dup: SeqSplitter[A] = {
                  val pit = new ParRRBVectorIterator_c_256_asserted(_end.-(remaining), _end);
                  pit.initIteratorFrom(this);
                  pit
                };
                def split: Seq[ParRRBVectorIterator_c_256_asserted] = {
                  val rem = remaining;
                  if (rem.>=(2))
                    {
                      val splitSize = (1).<<((5).*((31).-(java.lang.Integer.numberOfLeadingZeros(rem))./(8).-(1)));
                      val splitted = new ArrayBuffer[ParRRBVectorIterator_c_256_asserted]();
                      var currentPos = _end.-(remaining);
                      while (currentPos.<(rem)) 
                        {
                          val pit = new ParRRBVectorIterator_c_256_asserted(currentPos, math.min(currentPos.+(splitSize), _end));
                          pit.initIteratorFrom(this);
                          splitted.+=(pit);
                ...
                def psplit(sizes: Int*): Seq[ParRRBVectorIterator_c_256_asserted] = {
                  val splitted = new ArrayBuffer[ParRRBVectorIterator_c_256_asserted]();
                  var currentPos = _end.-(remaining);
                  sizes.foreach(((sz) => {
                    val pit = new ParRRBVectorIterator_c_256_asserted(currentPos, currentPos.+(sz));
                    pit.initIteratorFrom(this);
                    splitted.+=(pit);
                    currentPos.+=(sz)
                  }));
            ...
            object ParRRBVector_c_256_asserted extends ParFactory[ParRRBVector_c_256_asserted] {
              implicit def canBuildFrom[A]: CanCombineFrom[Coll, A, ParRRBVector_c_256_asserted[A]] = new GenericCanCombineFrom[A]();
              def newBuilder[A]: Combiner[A, ParRRBVector_c_256_asserted[A]] = newCombiner[A];
              def newCombiner[A]: Combiner[A, ParRRBVector_c_256_asserted[A]] = new ParRRBVectorCombinator_c_256_asserted[A]()
            }

            private[immutable] class ParRRBVectorCombinator_c_256_asserted[A] extends Combiner[A, ParRRBVector_c_256_asserted[A]] {
              val builder: RRBVectorBuilder_c_256_asserted[A] = new RRBVectorBuilder_c_256_asserted[A]();
              override def size = builder.endIndex;
              override def result() = new ParRRBVector_c_256_asserted[A](builder.result());
              ...
              def combine[B <: A, NewTo >: ParRRBVector_c_256_asserted[A]](other: Combiner[B, NewTo]) = if (other.eq(this))
                this
              else
                {
                  val that = other.asInstanceOf[ParRRBVectorCombinator_c_256_asserted[B]];
