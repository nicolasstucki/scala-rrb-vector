package scala {
    ...
    package immutable {
      package generated {
        package rrbvector.quick {
          ...
          object RRBVector_quick_32 extends scala.collection.generic.IndexedSeqFactory[RRBVector_quick_32] {
            def newBuilder[A]: mutable.Builder[A, RRBVector_quick_32[A]] = new RRBVector_quick_32Builder[A]();
            implicit def canBuildFrom[A]: scala.collection.generic.CanBuildFrom[Coll, A, RRBVector_quick_32[A]] = ReusableCBF.asInstanceOf[GenericCanBuildFrom[A]];
            lazy private val EMPTY_VECTOR = new RRBVector_quick_32[Nothing](0);
            override def empty[A]: RRBVector_quick_32[A] = EMPTY_VECTOR
          }

          final class RRBVector_quick_32[+A](override private[immutable] val endIndex: Int) extends scala.collection.AbstractSeq[A] with scala.collection.immutable.IndexedSeq[A] with scala.collection.generic.GenericTraversableTemplate[A, RRBVector_quick_32] with scala.collection.IndexedSeqLike[A, RRBVector_quick_32[A]] with RRBVector_quick_32Pointer[A @uncheckedVariance] with Serializable { self =>
            private[immutable] var dirty: Boolean = false;
            override def companion: scala.collection.generic.GenericCompanion[RRBVector_quick_32] = RRBVector_quick_32;
            def length(): Int = endIndex;
            override def lengthCompare(len: Int): Int = endIndex.-(len);
            private[collection] def initIterator[B >: A](it: RRBVector_quick_32Iterator[B]): scala.Unit = {
              if (this.dirty)
                {
                  this.stabilize();
                  this.dirty = false
            ...
            private[collection] def initIterator[B >: A](it: RRBVector_quick_32ReverseIterator[B]): scala.Unit = {
              if (this.dirty)
                {
                  this.stabilize();
                  this.dirty = false
            ...
            override def iterator: RRBVector_quick_32Iterator[A] = {
              val it = new RRBVector_quick_32Iterator[A](0, endIndex);
              this.initIterator(it);
              it
            };
            override def reverseIterator: RRBVector_quick_32ReverseIterator[A] = {
              val it = new RRBVector_quick_32ReverseIterator[A](0, endIndex);
              this.initIterator(it);
              it
            };
            ...
            override def :+[B >: A, That](elem: B)(implicit bf: CanBuildFrom[RRBVector_quick_32[A], B, That]): That = if (bf.eq(IndexedSeq.ReusableCBF))
              (if (this.endIndex.==(0))
  {
    val resultVector = new RRBVector_quick_32[B](1);
...
else
  {
    ...
    val resultVector = new RRBVector_quick_32[B](_endIndex.+(1));
    ...
    if (elemIndexInBlock.==(0))
    ...
    else
      {
        ...
        resultVector
            ...
            override def take(n: Int): RRBVector_quick_32[A] = if (n.<=(0))
              RRBVector_quick_32.empty
            else
              if (n.<(endIndex))
                takeFront0(n)
              else
                this;
            override def dropRight(n: Int): RRBVector_quick_32[A] = if (n.<=(0))
              this
            else
              if (n.<(endIndex))
                ...
                RRBVector_quick_32.empty;
            override def slice(from: Int, until: Int): RRBVector_quick_32[A] = take(until).drop(from);
            override def splitAt(n: Int): scala.Tuple2[RRBVector_quick_32[A], RRBVector_quick_32[A]] = scala.Tuple2(take(n), drop(n));
            override def ++[B >: A, That](that: GenTraversableOnce[B])(implicit bf: CanBuildFrom[RRBVector_quick_32[A], B, That]): That = if (bf.eq(IndexedSeq.ReusableCBF))
              if (that.isEmpty)
                this.asInstanceOf[That]
              else
                if (that.isInstanceOf[RRBVector_quick_32[B]])
                  {
                    val thatVec = that.asInstanceOf[RRBVector_quick_32[B]];
                    if (this.isEmpty)
                      thatVec.asInstanceOf[That]
                    else
                      {
                        val newVec = new RRBVector_quick_32(this.endIndex.+(thatVec.endIndex));
              ...
              super.++(that.seq);
            override def tail: RRBVector_quick_32[A] = if (this.endIndex.!=(0))
              this.drop(1)
            else
            ...
            override def init: RRBVector_quick_32[A] = if (this.endIndex.!=(0))
              dropRight(1)
            else
            ...
            private[immutable] def concatenate[B >: A](currentSize: Int, that: RRBVector_quick_32[B]): scala.Unit = {
              if (this.dirty)
                {
                  this.stabilize();
                  this.dirty = false
  ...
  32).+(1));
            ...
            private def takeFront0(n: Int): RRBVector_quick_32[A] = {
              if (dirty)
                {
                  stabilize();
                  dirty = false
                ...
                ();
              val vec = new RRBVector_quick_32[A](n);
          ...
          final class RRBVector_quick_32Builder[A] extends mutable.Builder[A, RRBVector_quick_32[A]] with RRBVector_quick_32Pointer[A @uncheckedVariance] {
            ...
            def result(): RRBVector_quick_32[A] = {
              val size = endIndex;
              if (size.==(0))
                RRBVector_quick_32.empty
              else
                {
                  val resultVector = new RRBVector_quick_32[A](size);
          ...
          class RRBVector_quick_32Iterator[+A](startIndex: Int, override private[immutable] val endIndex: Int) extends AbstractIterator[A] with Iterator[A] with RRBVector_quick_32Pointer[A @uncheckedVariance] {
          ...
          class RRBVector_quick_32ReverseIterator[+A](startIndex: Int, final override private[immutable] val endIndex: Int) extends AbstractIterator[A] with Iterator[A] with RRBVector_quick_32Pointer[A @uncheckedVariance] {
          ...
          private[immutable] trait RRBVector_quick_32Pointer[A] {
            ...
            final private[immutable] def initWithFocusFrom[U](that: RRBVector_quick_32[U]): Unit = {
            ...
            final private[immutable] def initFrom[U](that: RRBVector_quick_32Pointer[U]): Unit = {
              depth = that.depth;
              that.depth match {
                case 0 => ()
