package scala {
    ...
    package immutable {
      package generated {
        package rrbvector.quick.block128 {
          ...
          import scala.collection.parallel.immutable.generated.rrbvector.quick.block128.ParRRBVector_q_128

          object RRBVector_q_128 extends scala.collection.generic.IndexedSeqFactory[RRBVector_q_128] {
            def newBuilder[A]: mutable.Builder[A, RRBVector_q_128[A]] = new RRBVectorBuilder_q_128[A]();
            implicit def canBuildFrom[A]: scala.collection.generic.CanBuildFrom[Coll, A, RRBVector_q_128[A]] = ReusableCBF.asInstanceOf[GenericCanBuildFrom[A]];
            lazy private val EMPTY_VECTOR = new RRBVector_q_128[Nothing](0);
            override def empty[A]: RRBVector_q_128[A] = EMPTY_VECTOR
          }

          final class RRBVector_q_128[+A](override private[immutable] val endIndex: Int) extends scala.collection.AbstractSeq[A] with scala.collection.immutable.IndexedSeq[A] with scala.collection.generic.GenericTraversableTemplate[A, RRBVector_q_128] with scala.collection.IndexedSeqLike[A, RRBVector_q_128[A]] with RRBVectorPointer_q_128[A @uncheckedVariance] with Serializable { self =>
            private[immutable] var dirty: Boolean = false;
            override def companion: scala.collection.generic.GenericCompanion[RRBVector_q_128] = RRBVector_q_128;
            def length(): Int = endIndex;
            override def lengthCompare(len: Int): Int = endIndex.-(len);
            override def par = new ParRRBVector_q_128[A](this);
            override def iterator: RRBVectorIterator_q_128[A] = {
              if (this.dirty)
                {
                  this.stabilize();
                  this.dirty = false
                ...
                ();
              val it = new RRBVectorIterator_q_128[A](0, endIndex);
              it.initIteratorFrom(this);
              it
            };
            override def reverseIterator: RRBVectorReverseIterator_q_128[A] = {
              if (this.dirty)
                {
                  this.stabilize();
                  this.dirty = false
                ...
                ();
              val rit = new RRBVectorReverseIterator_q_128[A](0, endIndex);
            ...
            override def :+[B >: A, That](elem: B)(implicit bf: CanBuildFrom[RRBVector_q_128[A], B, That]): That = if (bf.eq(IndexedSeq.ReusableCBF))
              (if (this.endIndex.==(0))
  {
    val resultVector = new RRBVector_q_128[B](1);
...
else
  {
    ...
    val resultVector = new RRBVector_q_128[B](_endIndex.+(1));
    ...
    if (elemIndexInBlock.==(0))
    ...
    else
      {
        ...
        resultVector
            ...
            override def take(n: Int): RRBVector_q_128[A] = if (n.<=(0))
              RRBVector_q_128.empty
            else
              if (n.<(endIndex))
                takeFront0(n)
              else
                this;
            override def dropRight(n: Int): RRBVector_q_128[A] = if (n.<=(0))
              this
            else
              if (n.<(endIndex))
                ...
                RRBVector_q_128.empty;
            override def slice(from: Int, until: Int): RRBVector_q_128[A] = take(until).drop(from);
            override def splitAt(n: Int): scala.Tuple2[RRBVector_q_128[A], RRBVector_q_128[A]] = scala.Tuple2(take(n), drop(n));
            override def ++[B >: A, That](that: GenTraversableOnce[B])(implicit bf: CanBuildFrom[RRBVector_q_128[A], B, That]): That = if (bf.eq(IndexedSeq.ReusableCBF))
              if (that.isEmpty)
                this.asInstanceOf[That]
              else
                if (that.isInstanceOf[RRBVector_q_128[B]])
                  {
                    val thatVec = that.asInstanceOf[RRBVector_q_128[B]];
                    if (this.isEmpty)
                      thatVec.asInstanceOf[That]
                    else
                      {
                        val newVec = new RRBVector_q_128(this.endIndex.+(thatVec.endIndex));
              ...
              super.++(that.seq);
            override def tail: RRBVector_q_128[A] = if (this.endIndex.!=(0))
              this.drop(1)
            else
            ...
            override def init: RRBVector_q_128[A] = if (this.endIndex.!=(0))
              dropRight(1)
            else
            ...
            private[immutable] def concatenate[B >: A](currentSize: Int, that: RRBVector_q_128[B]): scala.Unit = {
              if (this.dirty)
                {
                  this.stabilize();
                  this.dirty = false
  ...
  128).+(1));
            ...
            private def takeFront0(n: Int): RRBVector_q_128[A] = {
              if (dirty)
                {
                  stabilize();
                  dirty = false
                ...
                ();
              val vec = new RRBVector_q_128[A](n);
          ...
          final class RRBVectorBuilder_q_128[A] extends mutable.Builder[A, RRBVector_q_128[A]] with RRBVectorPointer_q_128[A @uncheckedVariance] {
            display0 = new Array[AnyRef](128);
            depth = 1;
            private var blockIndex = 0;
            private var lo = 0;
            private var acc: RRBVector_q_128[A] = null;
            ...
            override def ++=(xs: TraversableOnce[A]): this.type = {
              if (xs.nonEmpty)
                if (xs.isInstanceOf[RRBVector_q_128[A]])
                  {
                    val thatVec = xs.asInstanceOf[RRBVector_q_128[A]];
                    if (endIndex.!=(0))
                      {
                        acc = this.result().++(xs);
                        this.clearCurrent()
            ...
            private def currentResult(): RRBVector_q_128[A] = {
              val size = blockIndex.+(lo);
              if (size.==(0))
                RRBVector_q_128.empty
              else
                {
                  val resultVector = new RRBVector_q_128[A](size);
          ...
          class RRBVectorIterator_q_128[+A](startIndex: Int, override private[immutable] val endIndex: Int) extends AbstractIterator[A] with Iterator[A] with RRBVectorPointer_q_128[A @uncheckedVariance] {
            private var blockIndex: Int = _;
            private var lo: Int = _;
            private var endLo: Int = _;
            private var _hasNext: Boolean = _;
            final private[collection] def initIteratorFrom[B >: A](that: RRBVectorPointer_q_128[B]): Unit = {
              if (_hasNext)
                {
                  focusOn(startIndex);
          ...
          class RRBVectorReverseIterator_q_128[+A](startIndex: Int, final override private[immutable] val endIndex: Int) extends AbstractIterator[A] with Iterator[A] with RRBVectorPointer_q_128[A @uncheckedVariance] {
          ...
          private[immutable] trait RRBVectorPointer_q_128[A] {
            ...
            final private[immutable] def initWithFocusFrom[U](that: RRBVectorPointer_q_128[U]): Unit = {
            ...
            final private[immutable] def initFrom[U](that: RRBVectorPointer_q_128[U]): Unit = {
              depth = that.depth;
              that.depth match {
                case 0 => ()
      ...
      package immutable {
        package generated {
          package rrbvector.quick.block128 {
            ...
            class ParRRBVector_q_128[+A](vector: RRBVector_q_128[A]) extends ParSeq[A] with GenericParTemplate[A, ParRRBVector_q_128] with ParSeqLike[A, ParRRBVector_q_128[A], RRBVector_q_128[A]] with Serializable {
              override def companion = ParRRBVector_q_128;
              def this() = {
                this(RRBVector_q_128.empty[A]);
                ()
              };
              def apply(idx: Int) = vector.apply(idx);
              def length = vector.length;
              def splitter: SeqSplitter[A] = {
                val pit = new ParRRBVectorIterator_q_128(0, vector.length);
                pit.initIteratorFrom(vector);
                pit
              };
              override def seq: RRBVector_q_128[A] = vector;
              override def toVector: Vector[A] = vector.toVector;
              class ParRRBVectorIterator_q_128(_start: Int, _end: Int) extends RRBVectorIterator_q_128[A](_start, _end) with SeqSplitter[A] {
                final override def remaining: Int = super.remaining;
                def dup: SeqSplitter[A] = {
                  val pit = new ParRRBVectorIterator_q_128(_end.-(remaining), _end);
                  pit.initIteratorFrom(this);
                  pit
                };
                def split: Seq[ParRRBVectorIterator_q_128] = {
                  val rem = remaining;
                  if (rem.>=(2))
                    {
                      val splitSize = (1).<<((5).*((31).-(java.lang.Integer.numberOfLeadingZeros(rem))./(7).-(1)));
                      val splitted = new ArrayBuffer[ParRRBVectorIterator_q_128]();
                      var currentPos = _end.-(remaining);
                      while (currentPos.<(rem)) 
                        {
                          val pit = new ParRRBVectorIterator_q_128(currentPos, math.min(currentPos.+(splitSize), _end));
                          pit.initIteratorFrom(this);
                          splitted.+=(pit);
                ...
                def psplit(sizes: Int*): Seq[ParRRBVectorIterator_q_128] = {
                  val splitted = new ArrayBuffer[ParRRBVectorIterator_q_128]();
                  var currentPos = _end.-(remaining);
                  sizes.foreach(((sz) => {
                    val pit = new ParRRBVectorIterator_q_128(currentPos, currentPos.+(sz));
                    pit.initIteratorFrom(this);
                    splitted.+=(pit);
                    currentPos.+=(sz)
                  }));
            ...
            object ParRRBVector_q_128 extends ParFactory[ParRRBVector_q_128] {
              implicit def canBuildFrom[A]: CanCombineFrom[Coll, A, ParRRBVector_q_128[A]] = new GenericCanCombineFrom[A]();
              def newBuilder[A]: Combiner[A, ParRRBVector_q_128[A]] = newCombiner[A];
              def newCombiner[A]: Combiner[A, ParRRBVector_q_128[A]] = new ParRRBVectorCombinator_q_128[A]()
            }

            private[immutable] class ParRRBVectorCombinator_q_128[A] extends Combiner[A, ParRRBVector_q_128[A]] {
              val builder: RRBVectorBuilder_q_128[A] = new RRBVectorBuilder_q_128[A]();
              override def size = builder.endIndex;
              override def result() = new ParRRBVector_q_128[A](builder.result());
              ...
              def combine[B <: A, NewTo >: ParRRBVector_q_128[A]](other: Combiner[B, NewTo]) = if (other.eq(this))
                this
              else
                {
                  val that = other.asInstanceOf[ParRRBVectorCombinator_q_128[B]];
