      ...
      package generated {
        package rrbvector.complete.block128 {
          ...
          final class RRBVector_c_128_asserted[+A](override private[immutable] val endIndex: Int) extends scala.collection.AbstractSeq[A] with scala.collection.immutable.IndexedSeq[A] with scala.collection.generic.GenericTraversableTemplate[A, RRBVector_c_128_asserted] with scala.collection.IndexedSeqLike[A, RRBVector_c_128_asserted[A]] with RRBVectorPointer_c_128_asserted[A @uncheckedVariance] with Serializable { self =>
            ...
            private def dropFront0(n: Int): RRBVector_c_128_asserted[A] = {
              if (transient)
                {
                  normalize(depth);
                  transient = false
                ...
                ();
              val vec = new RRBVector_c_128_asserted[A](this.endIndex.-(n));
              vec.initWithFocusFrom(this);
              if (vec.depth.>(1))
                {
                  vec.focusOn(n);
              ...
              if (xor.<(2097152))
                {
                  ...
                  display0 = new Array(1);
                  val _emptyTransientBlock = RRBVector_c_128_asserted.emptyTransientBlock;
