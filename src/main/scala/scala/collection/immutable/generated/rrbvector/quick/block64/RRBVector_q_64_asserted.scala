          ...
          package rrbvector.quick.block64 {
                  ...
                  if (rem.>=(2))
                    {
                      ...
                      while (currentPos.<(rem)) 
                        {
                          val pit = new ParRRBVectorIterator_q_64_asserted(currentPos, math.min(currentPos.+(splitSize), _end));
                          pit.initIteratorFrom(this);
                          splitted.+=(pit);
            ...
            private[immutable] class ParRRBVectorCombinator_q_64_asserted[A] extends Combiner[A, ParRRBVector_q_64_asserted[A]] {
              ...
              def combine[B <: A, NewTo >: ParRRBVector_q_64_asserted[A]](other: Combiner[B, NewTo]) = if (other.eq(this))
                ...
                {
                  val that = other.asInstanceOf[ParRRBVectorCombinator_q_64_asserted[B]];
