package scala {
    ...
    package immutable {
      package generated {
        package rrbvector.quick.block32 {
          ...
          import scala.collection.parallel.immutable.generated.rrbvector.quick.block32.ParRRBVector_q_32_asserted

          object RRBVector_q_32_asserted extends scala.collection.generic.IndexedSeqFactory[RRBVector_q_32_asserted] {
            def newBuilder[A]: mutable.Builder[A, RRBVector_q_32_asserted[A]] = new RRBVectorBuilder_q_32_asserted[A]();
            implicit def canBuildFrom[A]: scala.collection.generic.CanBuildFrom[Coll, A, RRBVector_q_32_asserted[A]] = ReusableCBF.asInstanceOf[GenericCanBuildFrom[A]];
            lazy private val EMPTY_VECTOR = new RRBVector_q_32_asserted[Nothing](0);
            override def empty[A]: RRBVector_q_32_asserted[A] = EMPTY_VECTOR;
            final lazy private[immutable] val emptyTransientBlock = new Array[AnyRef](2)
          }

          final class RRBVector_q_32_asserted[+A](override private[immutable] val endIndex: Int) extends scala.collection.AbstractSeq[A] with scala.collection.immutable.IndexedSeq[A] with scala.collection.generic.GenericTraversableTemplate[A, RRBVector_q_32_asserted] with scala.collection.IndexedSeqLike[A, RRBVector_q_32_asserted[A]] with RRBVectorPointer_q_32_asserted[A @uncheckedVariance] with Serializable { self =>
            private[immutable] var transient: Boolean = false;
            override def companion: scala.collection.generic.GenericCompanion[RRBVector_q_32_asserted] = RRBVector_q_32_asserted;
            def length(): Int = endIndex;
            override def lengthCompare(len: Int): Int = endIndex.-(len);
            override def par = new ParRRBVector_q_32_asserted[A](this);
            override def iterator: RRBVectorIterator_q_32_asserted[A] = {
              if (this.transient)
                {
                ...
                ();
              val it = new RRBVectorIterator_q_32_asserted[A](0, endIndex);
              it.initIteratorFrom(this);
              it
            };
            override def reverseIterator: RRBVectorReverseIterator_q_32_asserted[A] = {
              if (this.transient)
                {
                  this.normalize(this.depth);
                  this.transient = false;
                ...
                ();
              val rit = new RRBVectorReverseIterator_q_32_asserted[A](0, endIndex);
            ...
            override def :+[B >: A, That](elem: B)(implicit bf: CanBuildFrom[RRBVector_q_32_asserted[A], B, That]): That = if (bf.eq(IndexedSeq.ReusableCBF))
              {
                val _endIndex = this.endIndex;
                if (_endIndex.!=(0))
                  {
                    val resultVector = new RRBVector_q_32_asserted[B](_endIndex.+(1));
            ...
            override def take(n: Int): RRBVector_q_32_asserted[A] = if (n.<=(0))
              RRBVector_q_32_asserted.empty
            else
              if (n.<(endIndex))
                takeFront0(n)
              else
                this;
            override def drop(n: Int): RRBVector_q_32_asserted[A] = if (n.<=(0))
              this
            else
              if (n.<(endIndex))
                dropFront0(n)
                ...
                RRBVector_q_32_asserted.empty;
            override def dropRight(n: Int): RRBVector_q_32_asserted[A] = if (n.<=(0))
              this
            else
              if (n.<(endIndex))
                ...
                RRBVector_q_32_asserted.empty;
            override def takeRight(n: Int): RRBVector_q_32_asserted[A] = if (n.<=(0))
              RRBVector_q_32_asserted.empty
            else
              if (n.<(endIndex))
                ...
                this;
            override def slice(from: Int, until: Int): RRBVector_q_32_asserted[A] = take(until).drop(from);
            override def splitAt(n: Int): scala.Tuple2[RRBVector_q_32_asserted[A], RRBVector_q_32_asserted[A]] = scala.Tuple2(take(n), drop(n));
            override def ++[B >: A, That](that: GenTraversableOnce[B])(implicit bf: CanBuildFrom[RRBVector_q_32_asserted[A], B, That]): That = if (bf.eq(IndexedSeq.ReusableCBF))
              if (that.isEmpty)
                this.asInstanceOf[That]
              else
                if (that.isInstanceOf[RRBVector_q_32_asserted[B]])
                  {
                    val thatVec = that.asInstanceOf[RRBVector_q_32_asserted[B]];
                    if (this.endIndex.==(0))
                      thatVec.asInstanceOf[That]
                    else
                      {
                        val newVec = new RRBVector_q_32_asserted(this.endIndex.+(thatVec.endIndex));
              ...
              super.++(that.seq);
            override def tail: RRBVector_q_32_asserted[A] = if (this.endIndex.!=(0))
              this.drop(1)
            else
            ...
            override def init: RRBVector_q_32_asserted[A] = if (this.endIndex.!=(0))
              dropRight(1)
            else
            ...
            private def createSingletonVector[B](elem: B) = {
              val resultVector = new RRBVector_q_32_asserted[B](1);
            ...
            private[immutable] def concatenate[B >: A](currentSize: Int, that: RRBVector_q_32_asserted[B]): scala.Unit = {
  ...
  32).+(1));
            ...
            private def takeFront0(n: Int): RRBVector_q_32_asserted[A] = {
              if (transient)
                {
                  normalize(depth);
                  transient = false
                ...
                ();
              val vec = new RRBVector_q_32_asserted[A](n);
            ...
            private def dropFront0(n: Int): RRBVector_q_32_asserted[A] = {
              if (transient)
                {
                  normalize(depth);
                  transient = false
                ...
                ();
              val vec = new RRBVector_q_32_asserted[A](this.endIndex.-(n));
              vec.initWithFocusFrom(this);
              if (vec.depth.>(1))
                {
                  vec.focusOn(n);
  ...
  "<=6 <==> display6==null ".+:(x$45)
          ...
          final class RRBVectorBuilder_q_32_asserted[A] extends mutable.Builder[A, RRBVector_q_32_asserted[A]] with RRBVectorPointer_q_32_asserted[A @uncheckedVariance] {
            display0 = new Array[AnyRef](32);
            depth = 1;
            private var blockIndex = 0;
            private var lo = 0;
            private var acc: RRBVector_q_32_asserted[A] = null;
            ...
            override def ++=(xs: TraversableOnce[A]): this.type = {
              if (xs.nonEmpty)
                if (xs.isInstanceOf[RRBVector_q_32_asserted[A]])
                  {
                    val thatVec = xs.asInstanceOf[RRBVector_q_32_asserted[A]];
            ...
            def result(): RRBVector_q_32_asserted[A] = {
            ...
            private def currentResult(): RRBVector_q_32_asserted[A] = {
              val size = blockIndex.+(lo);
              if (size.==(0))
                RRBVector_q_32_asserted.empty
              else
                {
                  val resultVector = new RRBVector_q_32_asserted[A](size);
          ...
          class RRBVectorIterator_q_32_asserted[+A](startIndex: Int, override private[immutable] val endIndex: Int) extends AbstractIterator[A] with Iterator[A] with RRBVectorPointer_q_32_asserted[A @uncheckedVariance] {
            private var blockIndex: Int = _;
            private var lo: Int = _;
            private var endLo: Int = _;
            private var _hasNext: Boolean = _;
            final private[collection] def initIteratorFrom[B >: A](that: RRBVectorPointer_q_32_asserted[B]): Unit = {
          ...
          class RRBVectorReverseIterator_q_32_asserted[+A](startIndex: Int, final override private[immutable] val endIndex: Int) extends AbstractIterator[A] with Iterator[A] with RRBVectorPointer_q_32_asserted[A @uncheckedVariance] {
          ...
          private[immutable] trait RRBVectorPointer_q_32_asserted[A] {
            ...
            final private[immutable] def initWithFocusFrom[U](that: RRBVectorPointer_q_32_asserted[U]): Unit = {
            ...
            final private[immutable] def initFrom[U](that: RRBVectorPointer_q_32_asserted[U]): Unit = {
              assert(that.!=(null));
              depth = that.depth;
              that.depth match {
                case 0 => ()
              ...
              if (xor.<(32768))
                {
                  ...
                  display0 = new Array(1);
                  val _emptyTransientBlock = RRBVector_q_32_asserted.emptyTransientBlock;
      ...
      package immutable {
        package generated {
          package rrbvector.quick.block32 {
            ...
            class ParRRBVector_q_32_asserted[+A](vector: RRBVector_q_32_asserted[A]) extends ParSeq[A] with GenericParTemplate[A, ParRRBVector_q_32_asserted] with ParSeqLike[A, ParRRBVector_q_32_asserted[A], RRBVector_q_32_asserted[A]] with Serializable {
              override def companion = ParRRBVector_q_32_asserted;
              def this() = {
                this(RRBVector_q_32_asserted.empty[A]);
                ()
              };
              def apply(idx: Int) = vector.apply(idx);
              def length = vector.length;
              def splitter: SeqSplitter[A] = {
                val pit = new ParRRBVectorIterator_q_32_asserted(0, vector.length);
                pit.initIteratorFrom(vector);
                pit
              };
              override def seq: RRBVector_q_32_asserted[A] = vector;
              override def toVector: Vector[A] = vector.toVector;
              class ParRRBVectorIterator_q_32_asserted(_start: Int, _end: Int) extends RRBVectorIterator_q_32_asserted[A](_start, _end) with SeqSplitter[A] {
                final override def remaining: Int = super.remaining;
                def dup: SeqSplitter[A] = {
                  val pit = new ParRRBVectorIterator_q_32_asserted(_end.-(remaining), _end);
                  pit.initIteratorFrom(this);
                  pit
                };
                def split: Seq[ParRRBVectorIterator_q_32_asserted] = {
                  val rem = remaining;
                  if (rem.>=(2))
                    {
                      val _half = rem./(2);
                ...
                def psplit(sizes: Int*): Seq[ParRRBVectorIterator_q_32_asserted] = {
                  val splitted = new ArrayBuffer[ParRRBVectorIterator_q_32_asserted]();
                  var currentPos = _end.-(remaining);
                  sizes.foreach(((sz) => {
                    val pit = new ParRRBVectorIterator_q_32_asserted(currentPos, currentPos.+(sz));
                    pit.initIteratorFrom(this);
                    splitted.+=(pit);
                    currentPos.+=(sz)
                  }));
            ...
            object ParRRBVector_q_32_asserted extends ParFactory[ParRRBVector_q_32_asserted] {
              implicit def canBuildFrom[A]: CanCombineFrom[Coll, A, ParRRBVector_q_32_asserted[A]] = new GenericCanCombineFrom[A]();
              def newBuilder[A]: Combiner[A, ParRRBVector_q_32_asserted[A]] = newCombiner[A];
              def newCombiner[A]: Combiner[A, ParRRBVector_q_32_asserted[A]] = new ParRRBVectorCombinator_q_32_asserted[A]()
            }

            private[immutable] class ParRRBVectorCombinator_q_32_asserted[A] extends Combiner[A, ParRRBVector_q_32_asserted[A]] {
              val builder: RRBVectorBuilder_q_32_asserted[A] = new RRBVectorBuilder_q_32_asserted[A]();
              override def size = builder.endIndex;
              override def result() = new ParRRBVector_q_32_asserted[A](builder.result());
              ...
              def combine[B <: A, NewTo >: ParRRBVector_q_32_asserted[A]](other: Combiner[B, NewTo]) = if (other.eq(this))
                this
              else
                {
