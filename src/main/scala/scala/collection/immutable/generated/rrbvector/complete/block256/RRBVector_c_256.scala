          ...
          final class RRBVector_c_256[+A](override private[immutable] val endIndex: Int) extends scala.collection.AbstractSeq[A] with scala.collection.immutable.IndexedSeq[A] with scala.collection.generic.GenericTraversableTemplate[A, RRBVector_c_256] with scala.collection.IndexedSeqLike[A, RRBVector_c_256[A]] with RRBVectorPointer_c_256[A @uncheckedVariance] with Serializable { self =>
            ...
            override def take(n: Int): RRBVector_c_256[A] = if (n.<=(0))
              ...
              if (n.<(endIndex))
                ...
                this;
            override def drop(n: Int): RRBVector_c_256[A] = if (n.<=(0))
              this
            else
              if (n.<(endIndex))
                dropFront0(n)
            ...
            override def dropRight(n: Int): RRBVector_c_256[A] = if (n.<=(0))
              ...
              if (n.<(endIndex))
                ...
                RRBVector_c_256.empty;
            override def takeRight(n: Int): RRBVector_c_256[A] = if (n.<=(0))
              RRBVector_c_256.empty
            else
              if (n.<(endIndex))
            ...
            private def dropFront0(n: Int): RRBVector_c_256[A] = {
              if (transient)
                {
                  normalize(depth);
                  transient = false
                ...
                ();
              val vec = new RRBVector_c_256[A](this.endIndex.-(n));
              vec.initWithFocusFrom(this);
              if (vec.depth.>(1))
                {
                  vec.focusOn(n);
              ...
              if (xor.<(16777216))
                {
                  ...
                  display0 = new Array(1);
                  val _emptyTransientBlock = RRBVector_c_256.emptyTransientBlock;
