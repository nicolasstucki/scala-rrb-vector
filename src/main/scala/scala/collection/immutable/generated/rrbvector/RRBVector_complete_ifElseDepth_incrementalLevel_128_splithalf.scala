package scala {
    ...
    package immutable {
      package generated {
        package rrbvector {
          ...
          import scala.collection.parallel.immutable.generated.rrbvector.ParRRBVector_complete_ifElseDepth_incrementalLevel_128_splithalf

          object RRBVector_complete_ifElseDepth_incrementalLevel_128_splithalf extends scala.collection.generic.IndexedSeqFactory[RRBVector_complete_ifElseDepth_incrementalLevel_128_splithalf] {
            def newBuilder[A]: mutable.Builder[A, RRBVector_complete_ifElseDepth_incrementalLevel_128_splithalf[A]] = new RRBVector_complete_ifElseDepth_incrementalLevel_128_splithalfBuilder[A]();
            implicit def canBuildFrom[A]: scala.collection.generic.CanBuildFrom[Coll, A, RRBVector_complete_ifElseDepth_incrementalLevel_128_splithalf[A]] = ReusableCBF.asInstanceOf[GenericCanBuildFrom[A]];
            lazy private val EMPTY_VECTOR = new RRBVector_complete_ifElseDepth_incrementalLevel_128_splithalf[Nothing](0);
            override def empty[A]: RRBVector_complete_ifElseDepth_incrementalLevel_128_splithalf[A] = EMPTY_VECTOR
          }

          final class RRBVector_complete_ifElseDepth_incrementalLevel_128_splithalf[+A](override private[immutable] val endIndex: Int) extends scala.collection.AbstractSeq[A] with scala.collection.immutable.IndexedSeq[A] with scala.collection.generic.GenericTraversableTemplate[A, RRBVector_complete_ifElseDepth_incrementalLevel_128_splithalf] with scala.collection.IndexedSeqLike[A, RRBVector_complete_ifElseDepth_incrementalLevel_128_splithalf[A]] with RRBVector_complete_ifElseDepth_incrementalLevel_128_splithalfPointer[A @uncheckedVariance] with Serializable { self =>
            private[immutable] var dirty: Boolean = false;
            override def companion: scala.collection.generic.GenericCompanion[RRBVector_complete_ifElseDepth_incrementalLevel_128_splithalf] = RRBVector_complete_ifElseDepth_incrementalLevel_128_splithalf;
            def length(): Int = endIndex;
            override def lengthCompare(len: Int): Int = endIndex.-(len);
            override def par = new ParRRBVector_complete_ifElseDepth_incrementalLevel_128_splithalf[A](this);
            override def iterator: RRBVector_complete_ifElseDepth_incrementalLevel_128_splithalfIterator[A] = {
              if (this.dirty)
                {
                  this.stabilize();
                  this.dirty = false
                ...
                ();
              val it = new RRBVector_complete_ifElseDepth_incrementalLevel_128_splithalfIterator[A](0, endIndex);
              it.initIteratorFrom(this);
              it
            };
            override def reverseIterator: RRBVector_complete_ifElseDepth_incrementalLevel_128_splithalfReverseIterator[A] = {
              if (this.dirty)
                {
                  this.stabilize();
                  this.dirty = false
                ...
                ();
              val rit = new RRBVector_complete_ifElseDepth_incrementalLevel_128_splithalfReverseIterator[A](0, endIndex);
            ...
            override def :+[B >: A, That](elem: B)(implicit bf: CanBuildFrom[RRBVector_complete_ifElseDepth_incrementalLevel_128_splithalf[A], B, That]): That = if (bf.eq(IndexedSeq.ReusableCBF))
              (if (this.endIndex.==(0))
  {
    val resultVector = new RRBVector_complete_ifElseDepth_incrementalLevel_128_splithalf[B](1);
...
else
  {
    ...
    val resultVector = new RRBVector_complete_ifElseDepth_incrementalLevel_128_splithalf[B](_endIndex.+(1));
    ...
    if (elemIndexInBlock.==(0))
    ...
    else
      {
        ...
        resultVector
            ...
            override def take(n: Int): RRBVector_complete_ifElseDepth_incrementalLevel_128_splithalf[A] = if (n.<=(0))
              RRBVector_complete_ifElseDepth_incrementalLevel_128_splithalf.empty
            else
              if (n.<(endIndex))
                takeFront0(n)
              else
                this;
            override def dropRight(n: Int): RRBVector_complete_ifElseDepth_incrementalLevel_128_splithalf[A] = if (n.<=(0))
              this
            else
              if (n.<(endIndex))
                ...
                RRBVector_complete_ifElseDepth_incrementalLevel_128_splithalf.empty;
            override def slice(from: Int, until: Int): RRBVector_complete_ifElseDepth_incrementalLevel_128_splithalf[A] = take(until).drop(from);
            override def splitAt(n: Int): scala.Tuple2[RRBVector_complete_ifElseDepth_incrementalLevel_128_splithalf[A], RRBVector_complete_ifElseDepth_incrementalLevel_128_splithalf[A]] = scala.Tuple2(take(n), drop(n));
            override def ++[B >: A, That](that: GenTraversableOnce[B])(implicit bf: CanBuildFrom[RRBVector_complete_ifElseDepth_incrementalLevel_128_splithalf[A], B, That]): That = if (bf.eq(IndexedSeq.ReusableCBF))
              if (that.isEmpty)
                this.asInstanceOf[That]
              else
                if (that.isInstanceOf[RRBVector_complete_ifElseDepth_incrementalLevel_128_splithalf[B]])
                  {
                    val thatVec = that.asInstanceOf[RRBVector_complete_ifElseDepth_incrementalLevel_128_splithalf[B]];
                    if (this.isEmpty)
                      thatVec.asInstanceOf[That]
                    else
                      {
                        val newVec = new RRBVector_complete_ifElseDepth_incrementalLevel_128_splithalf(this.endIndex.+(thatVec.endIndex));
              ...
              super.++(that.seq);
            override def tail: RRBVector_complete_ifElseDepth_incrementalLevel_128_splithalf[A] = if (this.endIndex.!=(0))
              this.drop(1)
            else
            ...
            override def init: RRBVector_complete_ifElseDepth_incrementalLevel_128_splithalf[A] = if (this.endIndex.!=(0))
              dropRight(1)
            else
            ...
            private[immutable] def concatenate[B >: A](currentSize: Int, that: RRBVector_complete_ifElseDepth_incrementalLevel_128_splithalf[B]): scala.Unit = {
              if (this.dirty)
                {
                  this.stabilize();
                  this.dirty = false
            ...
            private def takeFront0(n: Int): RRBVector_complete_ifElseDepth_incrementalLevel_128_splithalf[A] = {
              if (dirty)
                {
                  stabilize();
                  dirty = false
                ...
                ();
              val vec = new RRBVector_complete_ifElseDepth_incrementalLevel_128_splithalf[A](n);
          ...
          final class RRBVector_complete_ifElseDepth_incrementalLevel_128_splithalfBuilder[A] extends mutable.Builder[A, RRBVector_complete_ifElseDepth_incrementalLevel_128_splithalf[A]] with RRBVector_complete_ifElseDepth_incrementalLevel_128_splithalfPointer[A @uncheckedVariance] {
            display0 = new Array[AnyRef](128);
            depth = 1;
            private var blockIndex = 0;
            private var lo = 0;
            private var acc: RRBVector_complete_ifElseDepth_incrementalLevel_128_splithalf[A] = null;
            ...
            override def ++=(xs: TraversableOnce[A]): this.type = {
              if (xs.nonEmpty)
                if (xs.isInstanceOf[RRBVector_complete_ifElseDepth_incrementalLevel_128_splithalf[A]])
                  {
                    val thatVec = xs.asInstanceOf[RRBVector_complete_ifElseDepth_incrementalLevel_128_splithalf[A]];
                    if (endIndex.!=(0))
                      {
                        acc = this.result().++(xs);
                        this.clearCurrent()
            ...
            def result(): RRBVector_complete_ifElseDepth_incrementalLevel_128_splithalf[A] = {
            ...
            private def currentResult(): RRBVector_complete_ifElseDepth_incrementalLevel_128_splithalf[A] = {
              val size = blockIndex.+(lo);
              if (size.==(0))
                RRBVector_complete_ifElseDepth_incrementalLevel_128_splithalf.empty
              else
                {
                  val resultVector = new RRBVector_complete_ifElseDepth_incrementalLevel_128_splithalf[A](size);
          ...
          class RRBVector_complete_ifElseDepth_incrementalLevel_128_splithalfIterator[+A](startIndex: Int, override private[immutable] val endIndex: Int) extends AbstractIterator[A] with Iterator[A] with RRBVector_complete_ifElseDepth_incrementalLevel_128_splithalfPointer[A @uncheckedVariance] {
            private var blockIndex: Int = _;
            private var lo: Int = _;
            private var endLo: Int = _;
            private var _hasNext: Boolean = _;
            final private[collection] def initIteratorFrom[B >: A](that: RRBVector_complete_ifElseDepth_incrementalLevel_128_splithalfPointer[B]): Unit = {
              if (_hasNext)
                {
                  focusOn(startIndex);
          ...
          class RRBVector_complete_ifElseDepth_incrementalLevel_128_splithalfReverseIterator[+A](startIndex: Int, final override private[immutable] val endIndex: Int) extends AbstractIterator[A] with Iterator[A] with RRBVector_complete_ifElseDepth_incrementalLevel_128_splithalfPointer[A @uncheckedVariance] {
          ...
          private[immutable] trait RRBVector_complete_ifElseDepth_incrementalLevel_128_splithalfPointer[A] {
            ...
            final private[immutable] def initWithFocusFrom[U](that: RRBVector_complete_ifElseDepth_incrementalLevel_128_splithalfPointer[U]): Unit = {
            ...
            final private[immutable] def initFrom[U](that: RRBVector_complete_ifElseDepth_incrementalLevel_128_splithalfPointer[U]): Unit = {
              depth = that.depth;
      ...
      package immutable {
        package generated {
          package rrbvector {
            ...
            class ParRRBVector_complete_ifElseDepth_incrementalLevel_128_splithalf[+A](vector: RRBVector_complete_ifElseDepth_incrementalLevel_128_splithalf[A]) extends ParSeq[A] with GenericParTemplate[A, ParRRBVector_complete_ifElseDepth_incrementalLevel_128_splithalf] with ParSeqLike[A, ParRRBVector_complete_ifElseDepth_incrementalLevel_128_splithalf[A], RRBVector_complete_ifElseDepth_incrementalLevel_128_splithalf[A]] with Serializable {
              override def companion = ParRRBVector_complete_ifElseDepth_incrementalLevel_128_splithalf;
              def this() = {
                this(RRBVector_complete_ifElseDepth_incrementalLevel_128_splithalf.empty[A]);
                ()
              };
              def apply(idx: Int) = vector.apply(idx);
              def length = vector.length;
              def splitter: SeqSplitter[A] = {
                val pit = new ParRRBVector_complete_ifElseDepth_incrementalLevel_128_splithalfIterator(0, vector.length);
                pit.initIteratorFrom(vector);
                pit
              };
              override def seq: RRBVector_complete_ifElseDepth_incrementalLevel_128_splithalf[A] = vector;
              override def toVector: Vector[A] = vector.toVector;
              class ParRRBVector_complete_ifElseDepth_incrementalLevel_128_splithalfIterator(_start: Int, _end: Int) extends RRBVector_complete_ifElseDepth_incrementalLevel_128_splithalfIterator[A](_start, _end) with SeqSplitter[A] {
                final override def remaining: Int = super.remaining;
                def dup: SeqSplitter[A] = {
                  val pit = new ParRRBVector_complete_ifElseDepth_incrementalLevel_128_splithalfIterator(_end.-(remaining), _end);
                  pit.initIteratorFrom(this);
                  pit
                };
                def split: Seq[ParRRBVector_complete_ifElseDepth_incrementalLevel_128_splithalfIterator] = {
                  val rem = remaining;
                  if (rem.>=(2))
                ...
                def psplit(sizes: Int*): Seq[ParRRBVector_complete_ifElseDepth_incrementalLevel_128_splithalfIterator] = {
                  val splitted = new ArrayBuffer[ParRRBVector_complete_ifElseDepth_incrementalLevel_128_splithalfIterator]();
                  var currentPos = _end.-(remaining);
                  sizes.foreach(((sz) => {
                    val pit = new ParRRBVector_complete_ifElseDepth_incrementalLevel_128_splithalfIterator(currentPos, currentPos.+(sz));
                    pit.initIteratorFrom(this);
                    splitted.+=(pit);
                    currentPos.+=(sz)
                  }));
            ...
            object ParRRBVector_complete_ifElseDepth_incrementalLevel_128_splithalf extends ParFactory[ParRRBVector_complete_ifElseDepth_incrementalLevel_128_splithalf] {
              implicit def canBuildFrom[A]: CanCombineFrom[Coll, A, ParRRBVector_complete_ifElseDepth_incrementalLevel_128_splithalf[A]] = new GenericCanCombineFrom[A]();
              def newBuilder[A]: Combiner[A, ParRRBVector_complete_ifElseDepth_incrementalLevel_128_splithalf[A]] = newCombiner[A];
              def newCombiner[A]: Combiner[A, ParRRBVector_complete_ifElseDepth_incrementalLevel_128_splithalf[A]] = new ParRRBVector_complete_ifElseDepth_incrementalLevel_128_splithalfCombinator[A]()
            }

            private[immutable] class ParRRBVector_complete_ifElseDepth_incrementalLevel_128_splithalfCombinator[A] extends Combiner[A, ParRRBVector_complete_ifElseDepth_incrementalLevel_128_splithalf[A]] {
              val builder: RRBVector_complete_ifElseDepth_incrementalLevel_128_splithalfBuilder[A] = new RRBVector_complete_ifElseDepth_incrementalLevel_128_splithalfBuilder[A]();
              override def size = builder.endIndex;
              override def result() = new ParRRBVector_complete_ifElseDepth_incrementalLevel_128_splithalf[A](builder.result());
              ...
              def combine[B <: A, NewTo >: ParRRBVector_complete_ifElseDepth_incrementalLevel_128_splithalf[A]](other: Combiner[B, NewTo]) = if (other.eq(this))
                this
              else
                {
                  val that = other.asInstanceOf[ParRRBVector_complete_ifElseDepth_incrementalLevel_128_splithalfCombinator[B]];
