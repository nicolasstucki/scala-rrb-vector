        ...
        package generated {
          package rrbvector.closedblocks.quickrebalance {
            ...
            trait GenRRBVectorClosedBlocksQuickRebalanceBenchmark[A] extends BaseVectorBenchmark[A] with GenRRBVectorClosedBlocksQuickRebalanceGenerator[A] {
              override def generateVectors(from: Int, to: Int, by: Int): org.scalameter.Gen[GenRRBVectorClosedBlocksQuickRebalance[A]] = sizes(from, to, by).map(((size) => GenRRBVectorClosedBlocksQuickRebalance.tabulate(size)(element)))
            }
