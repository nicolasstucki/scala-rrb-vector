        ...
        package generated {
          package rrbvector.fullblocks {
            ...
            trait GenRRBVectorFullBlocksBenchmark[A] extends BaseVectorBenchmark[A] with GenRRBVectorFullBlocksGenerator[A] {
              override def generateVectors(from: Int, to: Int, by: Int): org.scalameter.Gen[GenRRBVectorFullBlocks[A]] = sizes(from, to, by).map(((size) => GenRRBVectorFullBlocks.tabulate(size)(element)))
            }
